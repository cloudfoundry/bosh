...
module Bosh::AwsCloud
  ...
  class Cloud < Bosh::Cloud
    ...
    def create_stemcell(image_path, cloud_properties)
      ...
      with_thread_name("create_stemcell(#{image_path}...)") do
        begin
          ...
          cloud_error("could not determine aki") unless aki
    ...
    # finds the correct aki for the
    # in the current region
    def find_aki(arch, root_device_name)

      filters = []
      filters << {:name => "architecture", :values => [arch]}
      filters << {:name => "image-type", :values => ["kernel"]}
      filters << {:name => "owner-alias", :values => ["amazon"]}

      response = @ec2.client.describe_images(:filters => filters)

      # do nasty hackery to select boot device and version from the name string
      # e.g. pv-grub-hd00_1.03-x86_64.gz
