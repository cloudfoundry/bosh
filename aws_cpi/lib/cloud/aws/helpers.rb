    ...
    def wait_resource(resource, target_state, state_method = :status,
      ...
      loop do
        ...
        begin
          ...
          # sometimes when we upload a stemcell and proceed to create a VM from it,
          # AWS reports that the AMI is missing, but checking the console it is there,
          # so by retrying we catch that race condition
