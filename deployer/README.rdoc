...
Your configuration must live inside a 'deployments' directory and follow the convention of having a $name subdir containing $name.yml,
where $name is your deployment name.  For example:

  % find deployments -type f
  deployments/vcs01/vcs01.yml
  deployments/dev32/dev32.yml
  deployments/dev33/dev33.yml
...
Set the micro bosh deployment to work with:
  ...
  Deployment set to '/var/vcap/deployments/dev33/dev33.yml'
...
The status command will show the persisted state for a given micro bosh instance.
  ...
  Disk CID       1
  Deployment     /var/vcap/deployments/dev33/dev33.yml
