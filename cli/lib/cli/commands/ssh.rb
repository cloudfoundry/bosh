module Bosh::Cli::Command
  class Ssh < Base
    ...
    def cleanup_ssh(manifest_name, job, user, indexes, user_regex = nil)
      director.cleanup_ssh(manifest_name, job, user, user_regex, indexes ||= [])
    end
    ...
    def setup_ssh(job, index, password, options, &block)
      ...
      resource_id = director.setup_ssh(manifest_name, job, index, user, public_key, password)
      if resource_id.nil?
      ...
      begin
        ...
        results = JSON.parse(File.read(tmp_file))
      rescue Bosh::Cli::DirectorError => e
        err "Unable to get ssh ip information: #{e}"
      ensure
      ...
      if results
        ...
        cleanup_ssh(manifest_name, job, user, indexes)
      end
    end
      ...
      setup_ssh(job, options["index"], nil, options) do |results, user|
        with_gateway(options["gateway_host"], options["gateway_user"]) do |gateway|
          results.each do | result|
            err "Failed to setup ssh on #{result}" if result["status"] != "success"
    ...
    def cleanup(*args)
      ...
      cleanup_ssh(manifest_name, options["job"], nil, [options["index"]], "^#{SSH_USER_PREFIX}")
    end
  end
end
