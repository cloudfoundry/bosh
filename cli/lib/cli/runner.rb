  ...
  module Cli
    ...
    class Runner
      ...
      def run
        ...
        if @namespace && @action
        ...
        elsif @original_args.empty? || @original_args[0] == "help"
          if @original_args.size == 1
            say(help_message)
          else
            cmd_args = @original_args[1..-1]
          ...
          unknown_command(@original_args.join(" "))

          suggestions = command_suggestions(@original_args).map { |cmd| "bosh #{cmd.usage}" }
      ...
      def dispatch(command = nil)
        ...
        case command.route
        when Array
          @namespace, @action, args_range = command.route
        when Proc
          @namespace, @action, args_range = command.route.call(@args)
        else
          raise "Command definition is invalid, router should be an Array or Proc"
        end
      end
