  ...
  module Cli
    ...
    class ReleaseTarball
      ...
      def unpack
        ...
        result = execute("tar -C #{@unpack_dir} -xzf #{@tarball_path}")
        @unpacked = !result.failed?
      end
      ...
      def repack(remote_release)
        ...
        Dir.chdir(@unpack_dir) do
          ...
          result = execute("tar -czf #{repacked_path} .")
      ...
      def perform_validation(options = {})
        ...
        manifest["jobs"].each_with_index do |job, i|
          ...
          if job_exists
            ...
            result = execute("tar -C #{job_tmp_dir} -xzf #{job_file}")
            job_extracted = !result.failed?
