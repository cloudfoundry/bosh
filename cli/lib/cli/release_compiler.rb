module Bosh::Cli
  ...
  class ReleaseCompiler
    ...
    def initialize(manifest_file, blobstore, remote_release = nil, release_dir = nil)
      ...
      if remote_release
        @remote_packages = remote_release["packages"].map { |pkg| OpenStruct.new(pkg) }
        @remote_jobs     = remote_release["jobs"].map { |job| OpenStruct.new(job) }
      else
        @remote_packages = [ ]
        @remote_jobs     = [ ]
      end
    ...
    def compile
      if exists?
        quit "Looks like you already have this version in `#{tarball_path.green}'"
      end
      ...
      @packages.each do |package|
        ...
        err("Cannot find package `#{package.name} (#{package.version})'") if package_filename.nil?
        FileUtils.cp(package_filename, File.join(@packages_dir, "#{package.name}.tgz"), :preserve => true)
      end
      ...
      @jobs.each do |job|
        ...
        err("Cannot find job `#{job.name} (#{job.version})") if job_filename.nil?
        FileUtils.cp(job_filename, File.join(@jobs_dir, "#{job.name}.tgz"), :preserve => true)
      end

      header "Building tarball"
      Dir.chdir(@build_dir) do
        result = execute("tar -czf #{tarball_path} . 2>&1")
        raise InvalidRelease, "Cannot create release tarball: #{result.stdout.join("\n")}" if result.failed?
    ...
    def tarball_path
      @tarball_path || File.join(File.dirname(@manifest_file), "#{@name}-#{@version}.tgz")
    end

    def find_package(package)
      final_index = VersionsIndex.new(File.join(@release_dir, ".final_builds", "packages", package.name))
      dev_index   = VersionsIndex.new(File.join(@release_dir, ".dev_builds", "packages", package.name))
      find_in_indices(final_index, dev_index, package)
    end

    def find_job(job)
      final_index = VersionsIndex.new(File.join(@release_dir, ".final_builds", "jobs", job.name))
      dev_index   = VersionsIndex.new(File.join(@release_dir, ".dev_builds", "jobs", job.name))
    ...
    def find_in_indices(final_index, dev_index, object)
      ...
      if File.exists?(filename)
        ...
        err "#{desc} is corrupted locally" if Digest::SHA1.file(filename) != sha1
      elsif blobstore_id
    ...
    def remote_object_exists?(collection, local_object)
      collection.any? do |remote_object|
        remote_object.name == local_object.name && remote_object.version.to_s == local_object.version.to_s
      end
    end
