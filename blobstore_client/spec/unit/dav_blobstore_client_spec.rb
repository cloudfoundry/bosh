...
describe Bosh::Blobstore::DavBlobstoreClient do
  ...
  describe 'options' do
    ...
    it 'should set up authentication when present' do
      ...
      httpclient.should_receive(:get).with('http://localhost/88/foobar', {},
                                           {'Authorization' => 'Basic am9objpzbWl0aA=='}).and_return(response)
      client = Bosh::Blobstore::DavBlobstoreClient.new({'endpoint' => 'http://localhost',
                                                           'user' => 'john',
                                                           'password' => 'smith'})
      client.get('foobar')
    end
  ...
  describe 'operations' do
    let(:client) { Bosh::Blobstore::DavBlobstoreClient.new('endpoint' => 'http://localhost') }
