...
describe Bosh::Blobstore::AzureBlobstoreClient do
  def azure_storage_account_name
    ENV['STORAGE_ACCOUNT_NAME'] || 'asdfuhuru'
  end

  def azure_storage_access_key
    ENV['STORAGE_ACCESS_KEY'] || '5lb+5ZXxyN5zEcSGFUrVhTeBJYhKx2UwxbhmeFv+6usmNuxq0Tj0IgvIQEqcSJ0roHllg4xOQF+9ZMehldiznA=='
  end
  ...
  before(:all) do
    ...
    VCR.use_cassette('Create container') do
      @azure_blob_service.create_container(@container_name, public_access_level: 'blob')
    end
  end
  ...
  context 'Private access' do
    ...
    context 'Big objects' do
      ...
      xit 'should upload and download a big file' do
        ...
        lfile.length.should eq chunks * 10
        chunks.times do |i|
