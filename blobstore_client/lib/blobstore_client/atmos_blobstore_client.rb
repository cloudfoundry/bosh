  ...
  module Blobstore
    class AtmosBlobstoreClient < BaseClient
      ...
      def create_file(file)
        ...
        URI::escape(Base64.encode64(JSON.dump(:oid => object_id, :sig => signature(object_id))))
      end

      def get_file(object_id, file)
        object_info = parse_object_info(object_id)
        oid = object_info["oid"]
        sig = object_info["sig"]

        url = @atmos_options[:url] + "/rest/objects/#{oid}?uid=#{URI::escape(@atmos_options[:uid])}" +
      ...
      def parse_object_info(object_id)
        begin
