...
if [[ ! -x ${bosh_app_dir}/bosh/bin/ruby ]]
then
  cd /tmp
  ...
  echo Ruby ${bosh_app_dir}/bosh/bin/ruby already installed, skipping installation.
fi

if [[ ! -x ${bosh_app_dir}/bosh/bin/gem ]]
then
  ...
  echo RubyGems ${bosh_app_dir}/bosh/bin/gem already installed, skipping installation.
fi
