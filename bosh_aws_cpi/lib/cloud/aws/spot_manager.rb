  ...
  class SpotManager
    ...
    def wait_for_spot_instance_request_to_be_active(spot_instance_requests)
      begin
        ...
        Bosh::Common.retryable(sleep: (total_spot_instance_request_wait_time/10), tries: 10, on: errors) do |tries, error|
          ...
          status = get_spot_instance_request_status(spot_instance_requests)

          # Failure states+status.  If any of these occur we should give up, since waiting won't help
          if status[:state] == 'failed' 
            fail_spot_creation(spot_instance_requests, "VM spot instance creation failed: #{status.inspect}") end
          if status[:state] == 'open' and (status[:status] != nil and status[:status][:code] == "price-too-low")
            fail_spot_creation(spot_instance_requests, "Cannot create VM spot instance because bid price is too low: #{status.inspect}") end
        ...
        cancel_pending_spot_requests(spot_instance_requests)
    ...
    def get_spot_instance_request_status(spot_instance_requests)
      ...
      response = @region.client.describe_spot_instance_requests( \
        :spot_instance_request_ids => get_spot_instance_request_ids(spot_instance_requests) )
      status = response[:spot_instance_request_set][0] #There is only ever 1 
      @logger.debug("Spot instance request status: #{status.inspect}")
      status
    end

    def fail_spot_creation(spot_instance_requests, message)
      @logger.error(message)
      cancel_pending_spot_requests(spot_instance_requests)
      raise Bosh::Clouds::VMCreationFailed.new(false), message
    end

    def get_spot_instance_request_ids(spot_instance_requests)
      spot_instance_requests[:spot_instance_request_set].map { |r| r[:spot_instance_request_id] } 
    end

    def cancel_pending_spot_requests(spot_instance_requests)
      @logger.warn("Failed to create spot instance: #{spot_instance_requests.inspect}.  Cancelling request...")
      cancel_response = @region.client.cancel_spot_instance_requests(:spot_instance_request_ids => get_spot_instance_request_ids(spot_instance_requests))
