  ...
  module Aws
    class Migrator
      ...
      def migrate
        ...
        migrations_to_run = migrations - environment_migrations
        migrations_to_run.each do |migration|
          migration.load_class.new(@aws_config).execute
        end
      end
      ...
      def aws_s3
        @aws_s3 ||= Bosh::Aws::S3.new(@aws_config['aws'])
      end
      ...
      def bucket_name
        "#{@aws_config['name']}-bosh-artifacts"
      end
    ...
    class Migration
      ...
      attr_reader :s3, :elb, :ec2, :rds, :route53, :logger
