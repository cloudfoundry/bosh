...
namespace :stemcell2 do
  ...
  def default_options(args)
    {
      :system_parameters_infrastructure => args[:infrastructure],
      :stemcell_name => ENV["STEMCELL_NAME"],
      :stemcell_version => ENV["STEMCELL_VERSION"],
      :stemcell_infrastructure => args[:infrastructure],
  ...
  def build(spec, options)
    ...
    cp_r File.expand_path("../misc/stemcell/build2", __FILE__), build_path
