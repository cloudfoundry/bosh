...
describe Bosh::Agent::Message::Apply do
  ...
  it 'should install a job with a .monit file' do
    ...
    monitrc = File.join(Bosh::Agent::Config.base_dir, 'data', 'jobs', 'hubba', '77', '00_hubba.hubba_hubba.monitrc')
  ...
  it 'should install two jobs with two .monit files' do
    ...
    @httpclient.should_receive(:get).with(*get_args).and_yield(
        job_data).and_return(response)
    @httpclient.should_receive(:get).with(*get_args2).and_yield(
    ...
    monitrc1 = File.join(Bosh::Agent::Config.base_dir, 'data', 'jobs', 'hubba', '77', '01_hubba.hubba_hubba.monitrc')
    File.exist?(monitrc1).should == true

    monitrc2 = File.join(Bosh::Agent::Config.base_dir, 'data', 'jobs', 'hubba2', '77', '00_hubba.hubba2_hubba.monitrc')
