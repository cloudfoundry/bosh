...
describe Bosh::Agent::Message::Ssh do
  def ssh_create_args
    ["setup", {"user" => "spec_user", "password" => "test", "public_key" => "PUBLIC"}.to_json]
  end

  def ssh_cleanup_args
    ["cleanup", {"user" => "root", "password" => "test", "public_key" => "PUBLIC"}.to_json]
  end
  ...
  it "should copy public key into users director" do
    ...
    ssh.stub!(:shell_cmd)
    rtn = ssh.start
    authorized_keys =  File.read(File.join(Bosh::Agent::Config.system_root, Bosh::Agent::Message::Ssh::SSH_USER_BASE_DIR,
                                 "spec_user/.ssh/authorized_keys"))
