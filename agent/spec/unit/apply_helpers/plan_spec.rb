require File.expand_path("../../../spec_helper", __FILE__)

describe Bosh::Agent::ApplyHelpers::Plan do

  before :each do
    @base_dir = Dir.mktmpdir
    Bosh::Agent::Config.base_dir = @base_dir
  end

  def make_plan(*args)
    Bosh::Agent::ApplyHelpers::Plan.new(*args)
  end

  let(:valid_spec) do
    {
      "deployment" => "mycloud",
      "configuration_hash" => "deadbeef",
      "job" => {
        "name" => "ccdb",
        "template" => "postgres",
        "version" => "42",
        "sha1" => "deadbeef",
        "blobstore_id" => "deadcafe"
      },
      "packages" => {
        "foo" => {
          "name" => "foo",
          "version" => "5.dev",
          "sha1" => "cadada",
          "blobstore_id" => "deadbad"
        },
        "bar" => {
          "name" => "bar",
          "version" => "15",
          "sha1" => "fafaeda",
          "blobstore_id" => "badeff"
        }
      }
    }
  end

  describe "initialization" do
    it "expects Hash argument" do
      expect {
        make_plan("test")
      }.to raise_error(ArgumentError, "Invalid spec format, " +
                                      "Hash expected, String given")
    end

    it "initializes deployment, job and packages" do
      plan = make_plan(valid_spec)

      plan.deployment.should == "mycloud"
      plan.job.should be_kind_of Bosh::Agent::ApplyHelpers::Job
      plan.packages.size.should == 2
      plan.packages.each do |package|
        package.should be_kind_of Bosh::Agent::ApplyHelpers::Package
      end

      plan.has_job?.should be_true
      plan.has_packages?.should be_true
      plan.configured?.should be_true
    end
  end

  describe "operations" do

    let(:plan) do
      make_plan(valid_spec)
    end

    it "installs job" do
      plan.job.should_receive(:install)
      plan.install_job
    end

    it "installs packages" do
      plan.packages.each do |package|
        package.should_receive(:install_for_job).with(plan.job)
      end

      plan.install_packages
    end

    it "configures job" do
      plan.job.should_receive(:configure)
      plan.configure_job
    end

  end

end
