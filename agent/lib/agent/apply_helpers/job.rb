  ...
  module ApplyHelpers
    class Job
      ...
      def install_job_monitrc(template_path, label)
        ...
        # Monit will load all {base_dir}/monit/*.monitrc files, so we
        # need to be careful when we clean them up. To avoid cleanup
        # ambiguity we prefix job monitrc files with job_
