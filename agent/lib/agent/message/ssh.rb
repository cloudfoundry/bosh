  ...
  module Message
    class Ssh < Base
      ...
      def initialize(args)
        ...
        @base_dir = File.join(Bosh::Agent::Config.system_root, SSH_USER_BASE_DIR)
      end
      ...
      def setup
        begin
          ...
          shell_cmd(%Q[mkdir -p #{@base_dir}])

          if password
            shell_cmd(%Q[useradd -m -b #{@base_dir} -s /bin/bash -p '#{encrypt_password(password)}' #{user}])
          else
            shell_cmd(%Q[useradd -m -b #{@base_dir} -s /bin/bash #{user}])
          end
          ...
          ssh_dir = File.join(@base_dir, user, ".ssh")
