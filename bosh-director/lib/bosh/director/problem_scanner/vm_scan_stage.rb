  ...
  class ProblemScanner::VmScanStage
    ...
    def scan(vms=nil)
      if vms
        ...
        instances = Models::Instance.filter(deployment: @deployment).all.select{|instance| expects_vm?(instance)}
      end
    ...
    def expects_vm?(instance)
      InstanceDecorator.new(instance).lifecycle == 'service' && ['started', 'stopped'].include?(instance.state)
    end
