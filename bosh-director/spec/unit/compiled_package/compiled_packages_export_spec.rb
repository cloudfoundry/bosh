    ...
    describe '#extract' do
      ...
      it 'yields the packages' do
        ...
        export.extract do |_, packages|
          ...
          expect(package1.blob_path).to eq File.join(tmp_dir, 'compiled_packages', 'blobs', package1.blobstore_id)
          ...
          expect(package2.blob_path).to eq File.join(tmp_dir, 'compiled_packages', 'blobs', package2.blobstore_id)
        end
      end

      it 'cleans up' do
        export = described_class.new(file: exported_tar)
        export.extract { expect(Dir.exist?(tmp_dir)).to be(true) }
