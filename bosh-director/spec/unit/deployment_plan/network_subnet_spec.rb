  ...
  describe "Bosh::Director::DeploymentPlan::NetworkSubnet with #{ip_provider}" do
    ...
    describe :allocate_dynamic_ip do
      ...
      it "should not allocate from the static pool" do
        ...
        expect(subnet.allocate_dynamic_ip).to eq(NetAddr::CIDR.create('192.168.0.1').to_i)
        expect(subnet.allocate_dynamic_ip).to eq(NetAddr::CIDR.create('192.168.0.2').to_i)
        expect(subnet.allocate_dynamic_ip).to eq(NetAddr::CIDR.create('192.168.0.3').to_i)
        expect(subnet.allocate_dynamic_ip).to eq(NetAddr::CIDR.create('192.168.0.5').to_i)
        expect(subnet.allocate_dynamic_ip).to be_nil
      end
