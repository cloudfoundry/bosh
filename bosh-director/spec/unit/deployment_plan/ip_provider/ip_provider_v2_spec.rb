      ...
      describe :release do
        ...
        context 'when reservation has an IP' do
          ...
          context 'when the IP is from a previous deploy and no longer in any subnets range' do
            it 'should release IP' do
              network_spec['subnets'].first['range'] = '192.168.6.0/30'
              network_spec['subnets'].first['gateway'] = '192.168.6.1'
              network_spec['subnets'].first['dns'] = []
          ...
          context 'when IP belongs to subnet' do
            ...
            context 'when it is a static reservation' do
              ...
              it 'should fail to reserve the IP if it was already reserved' do
                other_instance = double(:instance, model: Bosh::Director::Models::Instance.make, availability_zone: BD::DeploymentPlan::AvailabilityZone.new('az-2', {}))
                original_static_network_reservation = static_network_reservation
                new_static_network_reservation = BD::StaticNetworkReservation.new(other_instance, manual_network, NetAddr::CIDR.create('192.168.1.5').to_i)
