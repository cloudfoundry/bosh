...
describe Bosh::Director::DeploymentPlan::Job do
  ...
  describe '#bind_unallocated_vms' do
    ...
    it 'allocates a VM to all non obsolete instances if they are not already bound to a VM' do
      ...
      instance0 = BD::DeploymentPlan::Instance.new(job, 6, 'started', nil, {}, az, true, logger)
      instance1 = BD::DeploymentPlan::Instance.new(job, 6, 'started', nil, {}, az, false, logger)
  ...
  describe 'has_static_ips?' do
    before do
      allow(plan).to receive(:properties).with(no_args).and_return({})
      allow(plan).to receive(:release).with('release1').and_return(release)
    end

    context 'when any one of the networks has static IPs assigned' do
      let(:spec) do
        {
          'name' => 'job1',
          'template' => 'foo',
          ...
          'stemcell' => 'dea',
          'networks'  => [
            {'name' => 'fake-network-name', 'static_ips' => ['1.1.1.1']},
            {'name' => 'fake-network', 'default' => ['dns', 'gateway']}
          ],
        }
      end

      it 'returns true' do
        expect(job).to have_static_ips
      end
    end

    context 'when none of the networks has static IPs assigned (it is nil)' do
      let(:spec) do
        {
          'name' => 'job1',
          'template' => 'foo',
          ...
          'stemcell' => 'dea',
          'networks'  => [
            {'name' => 'fake-network-name', 'default' => ['dns', 'gateway'], 'static_ips' => nil},
            {'name' => 'fake-network'}
          ]
        }
      end
      ...
      it 'returns false' do
        expect(job).to_not have_static_ips
      end
    end
  end
end
