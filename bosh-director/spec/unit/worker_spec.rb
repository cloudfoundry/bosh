...
describe 'worker' do
  ...
  describe '#prep' do
    describe 'Delayed::Worker configuration' do
      it 'sets the max_attempts to 5' do
        worker.prep

        expect(Delayed::Worker.max_attempts).to eq(5)
      end

      context 'when workers is sent USR1' do
      ...
      context 'when deployment has director_pool' do
        let(:queues) { worker.queues }
