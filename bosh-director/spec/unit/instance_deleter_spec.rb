...
module Bosh::Director
  describe InstanceDeleter do
    ...
    shared_examples_for 'deleting instances' do
      before do
      ...
      it 'should delete the instances with the config max threads option' do
        ...
        delete_multiple_instances_with_no_options
      end
      ...
      it 'should delete the instances with the respected max threads option' do
        ...
        delete_multiple_instances_with_max_threads
      end
    ...
    describe '#delete_instances' do
      let(:instance) do
        instance_model = Models::Instance.make(vm: vm.model, deployment: deployment_model, uuid: 'uuid-1', job: 'fake-job-name', index: 5)
      ...
      let(:instances_to_delete) do
        instances = []
        5.times { instances << instance_double(Models::Instance, model: nil) }
        instances
      end

      it_behaves_like 'deleting instances' do
        let(:delete_instance) { deleter.delete_instances([instance], event_log_stage) }
        let(:delete_multiple_instances_with_max_threads) { deleter.delete_instances(instances_to_delete, event_log_stage, max_threads: 2) }
        let(:delete_multiple_instances_with_no_options) { deleter.delete_instances(instances_to_delete, event_log_stage) }
      end
    end
    ...
    describe '#delete_instance_plans' do
      ...
      it_behaves_like 'deleting instances' do
        let(:delete_instance) { deleter.delete_instance_plans([instance_plan], event_log_stage) }
        let(:delete_multiple_instances_with_max_threads) { deleter.delete_instance_plans(instance_plans_to_delete, event_log_stage, max_threads: 2) }
        let(:delete_multiple_instances_with_no_options) { deleter.delete_instance_plans(instance_plans_to_delete, event_log_stage) }
      end
    end
