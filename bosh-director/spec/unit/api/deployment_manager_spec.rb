    ...
    describe '#find_available' do
      context 'when there is a deployment with scope bosh.admin' do
        it 'should find this deployment when provided scope contains bosh.admin' do
          deployment = BD::Models::Deployment.make(name: 'admin-visible')

          expect(subject.find_available(%w(bosh.admin bosh.teams.prod.admin))).to eq [deployment]
        end
      end

      context 'when there is a deployment with non-admin scope' do
        it 'should find this deployment when provided scope contains bosh.admin' do
          deployment = BD::Models::Deployment.make(name: 'prod-team-visible', teams: 'prod')

          expect(subject.find_available(%w(bosh.admin bosh.teams.dev.admin))).to eq [deployment]
        end

        it 'should find this deployment when provided scope contains deployment scope' do
          deployment = BD::Models::Deployment.make(name: 'prod-team-visible', teams: 'prod')

          expect(subject.find_available(['bosh.teams.prod.admin'])).to eq [deployment]
        end
        ...
        it 'should not find this deployment when provided scope does not contain deployment scope and is not admin' do
          ...
          expect(subject.find_available(['bosh.teams.dev.admin'])).to be_empty
        end
      end
    end
  end
