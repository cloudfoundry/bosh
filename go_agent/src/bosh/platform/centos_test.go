...
func testCentosCreateUserWithPassword(t assert.TestingT, password string, expectedUseradd []string) {
...
func testCentosSetupManualNetworking(t assert.TestingT,
	deps centosDependencies,
	platform Platform) {
	networks := boshsettings.Networks{
		"bosh": boshsettings.Network{
...
func testCentosUnmountPersistentDisk(t assert.TestingT, isMounted bool) {
	deps, centos := buildCentos(1 * time.Millisecond)
...
func testCentosCalculateEphemeralDiskPartitionSizes(t assert.TestingT, totalMemInMb, diskSizeInMb, expectedSwap uint64) {
	deps, centos := buildCentos(1 * time.Millisecond)
...
func init() {
	Describe("Testing with Ginkgo", func() {
		...
		It("centos create user", func() {
			...
			testCentosCreateUserWithPassword(GinkgoT(), "bar-pwd", expectedUseradd)
		})
		It("centos create user with an empty password", func() {
			...
			testCentosCreateUserWithPassword(GinkgoT(), "", expectedUseradd)
		})
...
%sfoobar:...`,
		...
		It("centos setup dhcp", func() {
			...
			deps, centos := buildCentos(1 * time.Millisecond)
			testCentosSetupDhcp(GinkgoT(), deps, centos)

			assert.Equal(GinkgoT(), len(deps.cmdRunner.RunCommands), 1)
			assert.Equal(GinkgoT(), deps.cmdRunner.RunCommands[0], []string{"service", "network", "restart"})
		})
		...
		It("centos setup manual networking", func() {
			...
			testCentosSetupManualNetworking(GinkgoT(), deps, centos)

			time.Sleep(100 * time.Millisecond)

			assert.Equal(GinkgoT(), len(deps.cmdRunner.RunCommands), 7)
			assert.Equal(GinkgoT(), deps.cmdRunner.RunCommands[0], []string{"service", "network", "restart"})
			assert.Equal(GinkgoT(), deps.cmdRunner.RunCommands[1], []string{"arping", "-c", "1", "-U", "-I", "eth0", "192.168.195.6"})
			assert.Equal(GinkgoT(), deps.cmdRunner.RunCommands[6], []string{"arping", "-c", "1", "-U", "-I", "eth0", "192.168.195.6"})
		})
		...
		It("centos unmount persistent disk when not mounted", func() {

			testCentosUnmountPersistentDisk(GinkgoT(), false)
		})
		It("centos unmount persistent disk when already mounted", func() {

			testCentosUnmountPersistentDisk(GinkgoT(), true)
		})
		...
		It("centos calculate ephemeral disk partition sizes when disk is bigger than twice the memory", func() {
			...
			testCentosCalculateEphemeralDiskPartitionSizes(GinkgoT(), totalMemInMb, diskSizeInMb, expectedSwap)
		})
		...
		It("centos get file contents from c d r o m", func() {
		...
		It("centos get file contents from c d r o m when c d r o m failed to load", func() {
		...
		It("centos get file contents from c d r o m retries c d r o m reading", func() {
