...
module VMBuilder
  class Build < Thor
    ...
    method_options :debug => :boolean, :work_dir => Dir.pwd,
      :ovftool => "/usr/lib/vmware/ovftool/ovftool", :template => :string
    def micro(manifest, tarball)
      timeit "build" do
        Dir.mktmpdir do |tmpdir|
          ...
          copy_file(manifest, "#{tmpdir}/micro/config/micro.yml")
          copy_file(tarball, "#{tmpdir}/micro/config/micro.tgz")
    ...
    no_tasks do
      ...
      def vmbuilder_options(options)
        opts = {
          ...
          :proxy => "http://localhost:9999/ubuntu",
          :rootsize => "8192"
        }

        if options[:memory]
          opts[:mem] = options[:memory]
        end
        if options[:config]
