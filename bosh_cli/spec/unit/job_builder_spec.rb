...
describe Bosh::Cli::JobBuilder do
  with_release_directory do |dir|
    ...
    def make_builder(name, packages = [], templates = { }, built_packages = [],
    ...
    def add_templates(job_name, *files)
      ...
      files.each do |file|
        add_job(job_name, "templates/#{file}")
      end
    end
    ...
    it 'creates a new builder' do
      ...
      add_monit('foo')
      builder = make_builder('foo', ['foo', 'bar', 'baz'],
    ...
    it 'has a fingerprint' do
      add_templates('foo', 'a.conf', 'b.yml')
      add_monit('foo')
      builder = make_builder('foo', ['foo', 'bar'],
    ...
    it 'has a stable portable fingerprint' do
      add_templates('foo', 'a.conf', 'b.yml')
      add_monit('foo')
      b1 = make_builder('foo', ['foo', 'bar'],
                       ['a.conf', 'b.yml'], ['foo', 'bar'])
      f1 = b1.fingerprint
      expect(b1.reload.fingerprint).to eq(f1)
  
      b2 = make_builder('foo', ['foo', 'bar'],
                       ['a.conf', 'b.yml'], ['foo', 'bar'])
    ...
    it 'changes fingerprint when template files is changed' do
      ...
      add_job('foo', 'templates/a.conf', 'bzz')
    ...
    it 'whines if some templates are missing' do
      ...
      expect {
        make_builder('foo', [], ['a.conf', 'b.conf', 'c.conf'])
      }.to raise_error(Bosh::Cli::InvalidJob,
    ...
    it 'whines if some packages are missing' do
      expect {
        make_builder('foo', ['foo', 'bar', 'baz', 'app42'], { }, ['foo', 'bar'])
      }.to raise_error(Bosh::Cli::InvalidJob,
    ...
    it 'whines if there is no spec file' do
      expect {
        make_builder('foo', ['foo', 'bar', 'baz', 'app42'], { },
    ...
    it 'supports preparation script' do
      ...
      add_job('foo', 'prepare', script)
      script_path = release_dir.join('jobs', 'foo', 'prepare')
      FileUtils.chmod(0755, script_path)
      Bosh::Cli::JobBuilder.run_prepare_script(script_path)
  
      builder = make_builder('foo', ['bar', 'baz'], ['a.conf', 'b.yml'],
                            ['foo', 'bar', 'baz', 'app42'], false)
      ...
      Dir.chdir(builder.build_dir) do
        ...
        expect(File.read('job.MF')).to eq(File.read(
          release_dir.join('jobs', 'foo', 'spec')))
    ...
    it 'copies job files' do
      add_templates('foo', 'a.conf', 'b.yml')
      add_monit('foo')
      builder = make_builder('foo', ['foo', 'bar', 'baz', 'app42'],
                            ['a.conf', 'b.yml'], ['foo', 'bar', 'baz', 'app42'])
    ...
    it 'generates tarball' do
      ...
      builder = make_builder('foo', ['p1', 'p2'], ['bar', 'baz'], ['p1', 'p2'])
    ...
    it 'supports versioning' do
      ...
      builder = make_builder('foo', [], ['bar', 'baz'], [])
  
      v1_fingerprint = builder.fingerprint
      expect(release_dir).to_not have_file("/.dev_builds/jobs/foo/#{v1_fingerprint}.tgz")
      builder.build
      expect(release_dir).to have_file("/.dev_builds/jobs/foo/#{v1_fingerprint}.tgz")
  
      add_templates('foo', 'zb.yml')
      builder = make_builder('foo', [], ['bar', 'baz', 'zb.yml'], [])
      builder.build
      v2_fingerprint = builder.fingerprint

      expect(release_dir).to have_file("/.dev_builds/jobs/foo/#{v1_fingerprint}.tgz")
      expect(release_dir).to have_file("/.dev_builds/jobs/foo/#{v2_fingerprint}.tgz")
    ...
    it 'can point to either dev or a final version of a job' do
      ...
      job_name = 'foo'
      final_storage_dir = release_dir.join('.final_builds', 'jobs', job_name)
      ...
      dev_storage_dir = release_dir.join('.dev_builds', 'jobs', job_name)
      ...
      builder = make_builder(job_name, [], ['bar', 'baz'], [])
    ...
    it 'bumps major dev version in sync with final version' do
      ...
      final_builder = make_builder('foo', [], ['bar', 'baz'], [],
                                  true, true, blobstore)
      final_builder.build
      ...
      builder2 = make_builder('foo', [], ['bar', 'baz', 'bzz'], [])
      builder2.build
    ...
    it 'allows template subdirectories' do
      ...
      blobstore = double('blobstore')
      builder = make_builder('foo', [], ['foo/bar', 'bar/baz'],
    ...
    it 'supports dry run' do
      ...
      expect(release_dir).to_not have_file("/.final_builds/jobs/foo/#{v1_fingerprint}.tgz")
      ...
      expect(release_dir).to have_file("/.dev_builds/jobs/foo/#{v1_fingerprint}.tgz")
      expect(release_dir).to_not have_file("/.dev_builds/jobs/foo/#{v2_fingerprint}.tgz")
    end
  end
end
