...
module Bosh::Cli
  describe ReleaseBuilder do
    with_release_directory do |dir|
      ...
      let(:release) { Bosh::Cli::Release.new(release_dir.path) }

      before do
        release_dir.add_dir('config')
      end
      ...
      context 'when there is a final release' do
        it 'bumps the least significant segment for the next version' do
          final_storage_dir = release_dir.join('releases', release_name)
        ...
        it 'bumps the dev version matching the latest final release' do
          ...
          dev_storage_dir = release_dir.join('dev_releases', release_name)
      ...
      it 'builds a release' do
        ...
        expected_tarball_path = release_dir.join(
          'dev_releases',
          release_name,
      ...
      it 'allows building a new release when no content has changed' do
        release_path = File.join('dev_releases', release_name)
        expect(release_dir).to_not have_file("#{release_path}/#{release_name}-0+dev.1.tgz")

        new_builder.build
        expect(release_dir).to have_file("#{release_path}/#{release_name}-0+dev.1.tgz")
        expect(release_dir).to_not have_file("#{release_path}/#{release_name}-0+dev.2.tgz")

        new_builder.build
        expect(release_dir).to have_file("#{release_path}/#{release_name}-0+dev.2.tgz")
      end
  
      it 'errors when trying to re-create the same final version' do
        release_path = File.join('releases', release_name)
        new_builder({:version => '1', :final => true}).build
        expect(release_dir).to have_file("#{release_path}/#{release_name}-1.tgz")
