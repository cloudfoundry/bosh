...
describe Bosh::WardenCloud::Cloud do
  ...
  before :each do
    ...
    @cloud = WardenCloudHelper.new
    @cloud.wardencloud = Bosh::Clouds::Provider.create(:warden, options).delegate
    @env = @cloud.generate_agent_env @vm, DEFAULT_AGENT_ID, network_spec
  ...
  context do "set_agent_env"
    it "can set agent env" do
      @cloud.set_agent_env @vm.container_id, @env
    end
  end

  context do "get_agent_env"
    it "get agent env" do
      env = @cloud.get_agent_env @vm.container_id
      env["agent_id"].should == DEFAULT_AGENT_ID
      env["networks"]["nic1"]["ip"].should == "1.1.1.1"
      env["networks"]["nic1"]["type"].should == "static"
      env["vm"]["id"].should_not == nil
