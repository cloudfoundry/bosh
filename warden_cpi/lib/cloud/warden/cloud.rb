...
module Bosh::WardenCloud
  class Cloud < Bosh::Cloud
    ...
    def initialize(options)
      ...
      @db = DB.new(options["db"])
      @disk_manager = DiskManager.new(
        options["disk_dir"] || DEFAULT_DISK_DIR,
        setup_device_pool(options["device_pool"])
      )
    end
    ...
    def setup_device_pool(option)
      ...
      option["count"].times do |i|
        device_num = option["start_num"] + i
        pool.push(device_num) unless @db.device_ocuppied?(device_num)
      end
      DevicePool.new(pool)
    end
