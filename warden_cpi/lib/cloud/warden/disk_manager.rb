...
module Bosh::WardenCloud
  ...
  class DiskManager
    ...
    def create_disk(size)
      ...
      begin
        sh("sudo #{File.join(@script_dir, "create_disk.sh")} #{disk_path} #{device_path} #{device_num} > /dev/null 2>&1")
      rescue
        @device_pool.release(disk_id)
    ...
    def delete_disk(disk)
      ...
      sh("sudo #{File.join(@script_dir, "delete_disk.sh")} #{device_path} > /dev/null 2>&1")
