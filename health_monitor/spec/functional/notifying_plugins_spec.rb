...
describe 'notifying plugins' do
  ...
  def fake_director
    director = stub(Bosh::HealthMonitor::Director, :get_deployments => [])
    Bosh::HealthMonitor::Director.stub(:new => director)
  end

  let(:runner) { Bosh::HealthMonitor::Runner.new(spec_asset("dummy_plugin_config.yml")) }
  let(:event_processor) { Bosh::HealthMonitor.event_processor }
