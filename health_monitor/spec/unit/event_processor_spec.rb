...
describe Bhm::EventProcessor do
  ...
  it "logs and swallows plugin exceptions" do
    ...
    Bhm.logger.should_receive(:error).with("Plugin Bosh::HealthMonitor::Plugins::Logger failed to process alert: error1")
    Bhm.logger.should_receive(:error).with("Plugin Bosh::HealthMonitor::Plugins::Logger failed to process heartbeat: error2")
    @processor.process(:alert, alert_payload)
