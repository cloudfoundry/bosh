module Bosh::HealthMonitor
  module Events
    class Heartbeat < Base
      ...
      SERVICE_JOBS_PREFIXES = Set.new(%w(mysql mongodb redis rabbit postgresql vblob))
      ...
      def guess_role
        ...
        if @job.to_s.downcase =~ /(#{SERVICE_JOBS_PREFIXES.to_a.join('|')})_gateway$/i
          return "service"
        end

        # job name contains "_node"
        if @job.to_s.downcase =~ /(#{SERVICE_JOBS_PREFIXES.to_a.join('|')})_node(.*)/i
          return "service"
        end
