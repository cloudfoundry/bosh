...
then
  if [ -n "${OVFTOOL:-}" ]
  then
    image_vsphere_ovf_ovftool_path=$OVFTOOL
  fi
fi
...
# Abort when $image_vsphere_ovf_ovftool_path is not executable
if [ ! -x $image_vsphere_ovf_ovftool_path ]
then
  echo "$image_vsphere_ovf_ovftool_path is not executable"
  exit 1
fi
