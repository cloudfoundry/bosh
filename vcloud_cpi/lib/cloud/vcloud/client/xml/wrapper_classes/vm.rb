      ...
      class Vm < Wrapper
        ...
        def attach_disk_link
          get_nodes('Link', {'rel' => 'disk:attach',
            'type' => MEDIA_TYPE[:DISK_ATTACH_DETACH_PARAMS]}, true).pop
        end
        ...
        def hardware_section
          get_nodes('VirtualHardwareSection', nil, false,
            'http://schemas.dmtf.org/ovf/envelope/1').pop
        end

        def network_connection_section
          get_nodes('NetworkConnectionSection',
            {'type' => MEDIA_TYPE[:NETWORK_CONNECTION_SECTION]}).pop
        end
        ...
        def add_hard_disk(size_mb)
          ...
          raise "No SCSI controller found for VM #{name}" unless scsi_controller
          # Create a RASD item
        ...
        def set_nic_is_connected(nic_index, is_connected)
          ...
          raise "NIC #{nic_index} cannot be found on VM #{name}." unless
            connection
