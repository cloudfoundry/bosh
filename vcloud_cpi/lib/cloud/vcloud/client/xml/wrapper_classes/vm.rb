    ...
    module Xml
      class Vm < Wrapper
        ...
        def attach_disk_link
          get_nodes('Link', {'rel' => 'disk:attach', 'type' => MEDIA_TYPE[:DISK_ATTACH_DETACH_PARAMS]}, true).pop
        end

        def description
          node = get_nodes('Description')
        ...
        def detach_disk_link
          get_nodes('Link', {'rel' => 'disk:detach', 'type' => MEDIA_TYPE[:DISK_ATTACH_DETACH_PARAMS]}, true).pop
        end
        ...
        def reconfigure_link
          get_nodes('Link', {'rel' => 'reconfigureVm'}, true).pop
        end

        def insert_media_link
          get_nodes('Link', {'rel' => 'media:insertMedia'}, true).pop
        end

        def eject_media_link
          get_nodes('Link', {'rel' => 'media:ejectMedia'}, true).pop
        end

        def metadata_link
          get_nodes('Link', {'type' => MEDIA_TYPE[:METADATA]}, true).pop
        end
        ...
        def hardware_section
          get_nodes('VirtualHardwareSection', nil, false,'http://schemas.dmtf.org/ovf/envelope/1').pop
        end

        def network_connection_section
          get_nodes('NetworkConnectionSection', {'type' => MEDIA_TYPE[:NETWORK_CONNECTION_SECTION]}).pop
        end
        ...
        def add_hard_disk(size_mb)
          ...
          raise "No SCSI controller found for VM #{name}" if scsi_controller.nil?
          # Create a RASD item
          new_disk = WrapperFactory.create_instance('Item', nil, hardware_section.doc_namespaces)
          section.add_item(new_disk)
          # The order matters!
          new_disk.add_rasd(RASD_TYPES[:HOST_RESOURCE])
          new_disk.add_rasd(RASD_TYPES[:INSTANCE_ID])
          new_disk.add_rasd(RASD_TYPES[:RESOURCE_TYPE])
          new_disk.set_rasd(RASD_TYPES[:RESOURCE_TYPE], HARDWARE_TYPE[:HARD_DISK])
          host_resource = new_disk.get_rasd(RASD_TYPES[:HOST_RESOURCE])
          host_resource[new_disk.create_qualified_name('capacity', VCLOUD_NAMESPACE)] = size_mb.to_s
          host_resource[new_disk.create_qualified_name('busSubType', VCLOUD_NAMESPACE)] = scsi_controller.get_rasd_content(RASD_TYPES[:RESOURCE_SUB_TYPE])
          host_resource[new_disk.create_qualified_name('busType', VCLOUD_NAMESPACE)] = HARDWARE_TYPE[:SCSI_CONTROLLER]
        end
        ...
        def add_nic(nic_index, network_name, addressing_mode, ip = nil)
          ...
          new_nic = Xml::NicItemWrapper.new(Xml::WrapperFactory.create_instance('Item', nil, hardware_section.doc_namespaces))
          ...
          @logger.info("Adding NIC #{nic_index} to VM \"#{name}\" with the following parameters: "\
          "Network name: \"#{network_name}\", Addressing mode #{addressing_mode}, IP address: #{ip.nil? ? "blank" : ip}")
        end

        # NIC modification methods

        def connect_nic(nic_index, network_name, addressing_mode, ip_address  = nil)
          section = network_connection_section
          new_connection = WrapperFactory.create_instance('NetworkConnection', nil, network_connection_section.doc_namespaces)
          ...
          new_connection.ip_address = ip_address if !ip_address.nil?
        ...
        def set_nic_is_connected(nic_index, is_connected)
          ...
          raise "NIC #{nic_index} cannot be found on VM \"#{name}\"." if connection.nil?
