    ...
    module Xml
      class NetworkConnection < Wrapper
        ...
        def network_connection_index=(value)
          if get_nodes('NetworkConnectionIndex').pop.nil?
        ...
        def ip_address=(value)
          ...
          if get_nodes('IpAddress').pop.nil?
        ...
        def ip_address_allocation_mode=(value)
          raise "Invalid IP addressing mode. "  \
            "Valid IP addressing allocation modes are #{IP_ADDRESSING_MODE.values.join(' ')}" if !IP_ADDRESSING_MODE.values.include?(value)
