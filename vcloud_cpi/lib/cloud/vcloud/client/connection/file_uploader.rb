        ...
        class << self
          def upload(href, file, cookies = nil, http_method = :Put)
            ...
            net.start do |http|
              ...
              raise "Error Response: #{response.code}" if response.code.to_i >= 400
              response
            end
          end
          ...
          def create_request(href, file, cookies = nil, http_method = :Put)
            headers = cookies ? {"Cookie" => cookies.map { |(key, val)| "#{key.to_s}=#{CGI::unescape(val)}" }.sort.join(';')} : {}
            # Ruby 1.8 doesn't have size on the file object
