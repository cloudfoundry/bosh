  ...
  module Client
    module Connection
      class Connection
        ...
        def initialize(hostname_port, organization, request_timeout = nil, rest_client = nil, site = nil, file_uploader = nil)
          @organization = organization
          @logger = Config.logger
          @rest_logger = Config.rest_logger
          @rest_throttle = Config.rest_throttle
          rest_client = RestClient if rest_client.nil?
          rest_client.log = @rest_logger
          request_timeout = 60 if request_timeout.nil?
          @site = site.nil? ? rest_client::Resource.new(hostname_port, :timeout => request_timeout) : site
        ...
        def connect(username, password)
          ...
          response = @site['/api/sessions'].post({:Authorization=>auth_header_value, :Accept=>ACCEPT})
        ...
        def get(destination)
          @rest_logger.debug "#{__method__.to_s.upcase} #{delay}\t#{self.class.get_href(destination)}"
          sleep(delay)
          begin
        ...
        def post(destination, data, content_type = '*/*')
          @rest_logger.debug "#{__method__.to_s.upcase} #{delay}\t#{self.class.get_href(destination)}"
          sleep(delay)
          @rest_logger.debug("Warning: content type not specified.  Default to '*/*'") if content_type == '*/*'
        ...
        def put(destination, data, content_type = '*/*')
          @rest_logger.debug "#{__method__.to_s.upcase} #{delay}\t#{self.class.get_href(destination)}"
          sleep(delay)
          @rest_logger.debug("Warning: content type not specified.  Default to '*/*'") if content_type.nil?
          @rest_logger.debug("#{__method__.to_s.upcase} data:#{data.to_s}")
          begin
          ...
          @rest_logger.debug((response && !response.strip.empty?) ? response : "Received empty response.")
        ...
        # The PUT method in rest-client can't handle large files because it doesn't use the
        # underlying Net::HTTP body_stream attribute.  Without that, it won't use chunked
        # transfer-encoding.  It also reads in the whole file at once.
        ...
        def delay()
          @rest_throttle['min'] + rand(@rest_throttle['max'] - @rest_throttle['min'])
        end
