  ...
  module Client
    class Client
      ...
      def initialize(url, username, password, entities, control,
        ...
        # We assume the organization doesn't change often so we can get it at
        # login and cache it
      ...
      def upload_vapp_template(vapp_name, directory)
        ...
        is_ovf_directory = [:ovf_file, :ovf_file_path, :vmdk_file,
            :vmdk_file_path].reduce(true) do |p, n|
        ...
        log_exception(ex, "Error in uploading vApp template #{vapp_name}")
      ...
      def eject_catalog_media(vm, catalog_media_name)
        ...
        raise CatalogMediaNotFoundError,
          "Catalog media #{catalog_media_name} not found." unless
          catalog_media
        media = @connection.get(catalog_media.entity)
        current_vm = @connection.get(vm)
      ...
      rescue ArgumentError, ApiError => ex
        log_exception(ex, "Error uploading media #{media_name}" +
                    "to catalog #{catalog_name}. #{ex.message}")
      ...
      def delete_vapp(vapp)
        ...
        raise CloudError, "vApp #{vapp.name} is powered on, power-off before " +
          "deleting." if is_vapp_status(current_vapp, :POWERED_ON)
      ...
      def instantiate_vapp_template(source_template_id, vapp_name,
        ...
        @logger.error("Catalog item with ID #{source_template_id} not found " +
                      "in catalog #{@vapp_catalog_name}.") unless catalog_item
        raise ObjectNotFoundError, "Item with ID #{source_template_id} " +
              "not found in catalog #{@vapp_catalog_name}." unless catalog_item
      ...
      def attach_disk(disk, vm)
        ...
        current_disk = @connection.get(disk)
        raise ObjectNotFoundError, "Disk #{disk.name} not found." unless
          current_disk
      ...
      def power_on_vapp(vapp)
        ...
        current_vapp = @connection.get(vapp)
        raise ObjectNotFoundError, "vApp #{vapp.name} not found." unless
          current_vapp
        ...
        raise CloudError, "vApp #{vapp.name} not in a state be " +
          "powered on." unless current_vapp.power_on_link
      ...
      def power_off_vapp(vapp, undeploy = true)
        ...
        current_vapp = @connection.get(vapp)
        raise ObjectNotFoundError, "vApp #{vapp.name} no longer exists." unless
          current_vapp
        @logger.debug("vApp status: #{current_vapp['status']}")
        ...
        if undeploy
          # Since we don't apparently differentiate between powered-off and
      ...
      def get_ovdc
        ...
        raise ObjectNotFoundError, "Virtudal datacenter #{@ovdc_name} not " +
          "found." unless vdc
        @connection.get(vdc)
      end
      ...
      def copy_network_settings(network, network_config, vapp_net_name,
        ...
        config_ip_scope.start_address = net_ip_scope.start_address if
          net_ip_scope.start_address
        config_ip_scope.end_address = net_ip_scope.end_address if
          net_ip_scope.end_address
      ...
      def check_vapp_for_remove_link(vapp)
          current_vapp = @connection.get(vapp)
          raise ObjectNotFoundError, "No link available to delete vApp." unless
            current_vapp.remove_link
          return current_vapp
      end
            ...
            Util.retry_operation(vm.eject_media_link, @retries['default'],
              ...
              monitor_task(task) do |t|
                raise CloudError, "Error ejecting media #{media.name} from " +
                  "VM #{vm.name}." if t.status != 'success'
      ...
      def get_catalog_item(name, item_type, catalog_name)
        ...
        raise ObjectNotFoundError, "Catalog #{catalog_name} not found." unless
          @admin_org.catalog(catalog_name)
      ...
      def get_vm_network_connections(vm)
        ...
        raise ObjectNotFoundError,
          "VM #{vm.name} no longer exists." unless current_vm
        @connection.get(current_vm.network_connection_section)
      end

      def monitor_task(task, time_limit = @time_limit['default'],
          error_statuses = [Xml::TASK_STATUS[:ABORTED],
          Xml::TASK_STATUS[:ERROR], Xml::TASK_STATUS[:CANCELED]],
        ...
        while (i < iterations)
          @logger.debug("#{current_task.urn} #{current_task.operation} is " +
                        "#{current_task.status}")
          if success.map { |s| s.downcase }.find {
          ...
          elsif error_statuses.map { |s| s.downcase }.find {
              |s| s == current_task.status.downcase }
            raise ApiRequestError, "Task #{task.urn} #{task.operation} did " +
              "not complete successfully."
          elsif (current_task.progress &&
                 (current_task.progress != prev_progress)) ||
      ...
      def add_catalog_item(item, catalog_name)
        raise ArgumentError,
          "Error adding #{item.name}, catalog #{catalog_name} not found." unless
            @admin_org.catalog(catalog_name)
        catalog = @connection.get(@admin_org.catalog(catalog_name))
