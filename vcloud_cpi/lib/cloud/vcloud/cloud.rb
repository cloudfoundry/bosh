...
module VCloudCloud
  ...
  class Cloud
    ...
    def reconfigure_vm(vapp, resource_pool, networks, environment)
      ...
      raise IndexError, "Invalid number of VM hard disks" unless
        disks.size == 1
      system_disk = disks[0]
    ...
    def get_disk_size_mb(disk_id)
      ...
      log_exception("get_disk_size", ex)
      raise ex
    end
    ...
    def create_client()
      ...
      @client = VCloudCloud::Client::Client.new(url,@vcd["user"],
    ...
    def add_vapp_networks(vapp, networks)
      ...
      networks.map {|k,v| v["cloud_properties"]["name"]}.uniq.each do
          |configured_network|
        @logger.debug("Adding configured network: #{configured_network}")
        org_net = accessible_org_networks.find {
          |n| n['name'] == configured_network }
        raise CloudError, "Configured network: #{configured_network}, is " +
          "not accessible to VDC:#{ovdc.name}." unless org_net
        @logger.debug("Adding configured network: #{configured_network}, => " +
    ...
    def add_vm_nics(v, networks)
      ...
      networks.each_value do |network|
        ...
        v.add_nic(nic_index, configured_network,
          Client::Xml::IP_ADDRESSING_MODE[:MANUAL], network["ip"])
