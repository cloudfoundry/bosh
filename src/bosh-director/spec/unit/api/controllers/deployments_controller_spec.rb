  ...
  module Api
    describe Controllers::DeploymentsController do
      ...
      describe 'API calls' do
        describe 'creating a deployment' do
          context 'authenticated access' do
            ...
            it 'gives a nice error when request body is not a valid yml' do
              ...
              expect(JSON.parse(last_response.body)['code']).to eq(440_001)
              expect(JSON.parse(last_response.body)['description'])
                .to include('Incorrect YAML structure of the uploaded manifest: ')
            end
            ...
            context 'when provided cloud configs and runtime configs context to work within' do
              ...
              context 'with an authorized team' do
                ...
                it 'should error if the user references a cloud config from another team' do
                  deployment_context = [['context', JSON.dump('cloud_config_ids' => %w[dev_cloud_config.id other_cloud_config.id],
                                                              'runtime_config_ids' => [])]]

                  response = post "/?#{URI.encode_www_form(deployment_context)}", spec_asset('test_conf.yaml'),
                ...
                it 'should error if the user references a runtime config from another team' do
                  deployment_context = [['context',
                                         JSON.dump('cloud_config_ids' => [],
                                                   'runtime_config_ids' => %w[dev_runtime_config.id other_runtime_config.id])]]
                ...
                it 'should accept global and team-specific cloud and runtime configs' do
                  deployment_context = [['context',
                                         JSON.dump('cloud_config_ids' => [cloud_config.id, dev_cloud_config.id],
                                                   'runtime_config_ids' => [runtime_config_3.id, dev_runtime_config.id])]]
            ...
            context 'no existing context provided' do
              ...
              let(:runtime_config_3) do
                Models::Config.make(type: 'runtime', raw_manifest: { 'addons' => [] }, name: 'smurf')
              end
              let(:cloud_config) { Models::Config.make(:cloud, raw_manifest: { 'azs' => [] }) }
              ...
              let!(:dev_runtime_config) do
                Models::Config.make(name: 'dev-runtime', type: 'runtime', team_id: dev_team.id)
              end
              let!(:other_runtime_config) do
                Models::Config.make(name: 'other-runtime', type: 'runtime', team_id: other_team.id)
              end
              ...
              let!(:dev_cloud_config) do
                Models::Config.make(name: 'dev-cloud', type: 'cloud', team_id: dev_team.id)
              end
            ...
            it 'skips draining' do
              expect_any_instance_of(DeploymentManager)
                ...
                .with(anything, anything, anything, anything, anything,
                      hash_including('skip_drain' => 'job_one,job_two'), anything)
                .and_return(OpenStruct.new(id: 1))
        ...
        describe 'job management' do
          context 'when team-authorized' do
            ...
            it 'uses the correct configs when updating an instance' do
              ...
              put '/foo/jobs/dea/0B949287-CDED-4761-9002-FC4035E11B21?state=stopped', spec_asset('test_conf.yaml'),
                  'CONTENT_TYPE' => 'text/yaml'
              expect_redirect_to_queued_task(last_response)
            end
          end
          ...
          shared_examples 'change state' do
            it 'allows to change state' do
              ...
              Models::PersistentDisk.create(instance: instance, disk_cid: 'disk_cid')
              put path.to_s, spec_asset('test_conf.yaml'), 'CONTENT_TYPE' => 'text/yaml'
              expect_redirect_to_queued_task(last_response)
            end
            ...
            it 'allows to change state with content_length of 0' do
              ...
              Models::PersistentDisk.create(instance: instance, disk_cid: 'disk_cid')
              put path.to_s, manifest, 'CONTENT_TYPE' => 'text/yaml', 'CONTENT_LENGTH' => '0'
          ...
          it 'allows putting the job instance into different ignore state' do
            ...
            put '/foo/instance_groups/dea/0B949287-CDED-4761-9002-FC4035E11B21/ignore', JSON.generate('ignore' => true),
                'CONTENT_TYPE' => 'application/json'
            expect(last_response.status).to eq(200)
            expect(instance.reload.ignore).to be(true)

            put '/foo/instance_groups/dea/0B949287-CDED-4761-9002-FC4035E11B21/ignore', JSON.generate('ignore' => false),
                'CONTENT_TYPE' => 'application/json'
            expect(last_response.status).to eq(200)
          ...
          it 'should not validate body content when content.length is zero' do
            ...
            put '/foo/jobs/dea/0?state=started', "}}}i'm not really yaml, hah!", 'CONTENT_TYPE' => 'text/yaml',
                                                                                 'CONTENT_LENGTH' => '0'
            ...
            shared_examples_for 'recreates with configs' do
              it 'recreates with the latest configs if you send a manifest' do
                ...
                expect_any_instance_of(DeploymentManager)
                  ...
                  .and_return(OpenStruct.new(id: 1))
                put path.to_s, JSON.generate('value' => 'baz'), 'CONTENT_TYPE' => 'text/yaml'
                expect(last_response).to be_redirect
              end

              it 'recreates with the previous configs rather than the latest' do
                cc_old = Models::Config.create(name: 'cc', type: 'cloud', content: YAML.dump('foo' => 'old-cc'))
                Models::Config.create(name: 'cc', type: 'cloud', content: YAML.dump('foo' => 'new-cc'))
                runtime_old = Models::Config.create(name: 'runtime', type: 'runtime', content: YAML.dump('foo' => 'old-runtime'))
                Models::Config.create(name: 'runtime', type: 'runtime', content: YAML.dump('foo' => 'new-runtime'))
                ...
                expect_any_instance_of(DeploymentManager)
                  ...
                  .and_return(OpenStruct.new(id: 1))
                put path.to_s, '', 'CONTENT_TYPE' => 'text/yaml'
                expect(last_response).to be_redirect
              end
            end
          ...
          describe 'draining' do
            ...
            let(:instance) do
              Models::Instance.create(deployment: deployment, job: 'job_name', index: '0',
                                      uuid: '0B949287-CDED-4761-9002-FC4035E11B21', state: 'started',
                                      variable_set: Models::VariableSet.create(deployment: deployment))
            end
            before do
              Models::PersistentDisk.create(instance: instance, disk_cid: 'disk_cid')
            ...
            shared_examples 'skip_drain' do
              it 'drains' do
                ...
                put '/test_deployment/jobs/job_name/0B949287-CDED-4761-9002-FC4035E11B21', spec_asset('test_conf.yaml'),
                    'CONTENT_TYPE' => 'text/yaml'
                expect(last_response).to be_redirect
              end
              ...
              it 'skips draining' do
                expect_any_instance_of(DeploymentManager)
                  .to receive(:create_deployment)
                  .with(anything, anything, anything, anything, anything, hash_including('skip_drain' => drain_target.to_s))
                  .and_return(OpenStruct.new(id: 1))

                put (path + drain_option).to_s, spec_asset('test_conf.yaml'), 'CONTENT_TYPE' => 'text/yaml'
                expect(last_response).to be_redirect
              end
            end
        ...
        describe 'log management' do
          ...
          it '404 if no deployment' do
            Models::Deployment.create(name: 'bar', manifest: YAML.dump('foo' => 'bar'))
        ...
        describe 'listing deployments' do
          ...
          context 'with deployment info' do
            before do
              ...
              release11 = Models::ReleaseVersion.create(release: release1, version: 1)
              release12 = Models::ReleaseVersion.create(release: release1, version: 2)
              release2 = Models::Release.create(name: 'release-2')
              release21 = Models::ReleaseVersion.create(release: release2, version: 1)

              stemcell11 = Models::Stemcell.create(name: 'stemcell-1', version: 1, cid: 123)
              stemcell12 = Models::Stemcell.create(name: 'stemcell-1', version: 2, cid: 123)
              stemcell21 = Models::Stemcell.create(name: 'stemcell-2', version: 1, cid: 124)
              ...
              Models::Deployment.create(
                ...
                deployment.add_stemcell(stemcell11)
                deployment.add_stemcell(stemcell12)
                deployment.add_release_version(release11)
                deployment.add_release_version(release21)
              ...
              Models::Deployment.create(
                ...
                deployment.add_stemcell(stemcell11)
                deployment.add_stemcell(stemcell21)
                deployment.add_release_version(release11)
                deployment.add_release_version(release12)
          ...
          it 'orders the associations' do
            ...
            expect(body.first['releases']).to eq([{ 'name' => 'r1', 'version' => '2' }, { 'name' => 'r1', 'version' => '3' },
                                                  { 'name' => 'r2', 'version' => '1' }])
        ...
        describe 'getting deployment info' do
          ...
          it 'returns manifest' do
            Models::Deployment.create(name: 'test_deployment', manifest_text: YAML.dump('foo' => 'bar'))
            get '/test_deployment'

            expect(last_response.status).to eq(200)
            body = JSON.parse(last_response.body)
            expect(YAML.safe_load(body['manifest'])).to eq('foo' => 'bar')
          end
        end
          ...
          it 'returns a list of instances with vms (vm_cid != nil)' do
            8.times do |i|
              ...
              2.times do |j|
                ...
                instance.active_vm = vm if j.zero?
              end
            end
            ...
            body.sort_by { |instance| instance['agent_id'] }.each_with_index do |instance_with_vm, i|
              instance_idx = i / 2
              vm_by_instance = i % 2
              vm_is_active = vm_by_instance.zero?
          ...
          context 'with full format requested' do
            ...
            it 'redirects to a delayed job' do
              expect_any_instance_of(Api::InstanceManager).to receive(:fetch_instances_with_vm) do
                Bosh::Director::Models::Task.make(id: 10_002)
              end

              get '/test_deployment/vms?format=full'

              task = expect_redirect_to_queued_task(last_response)
              expect(task.id).to eq 10_002
            end
          end
        ...
        describe 'problem management' do
          ...
          it 'exposes problem managent REST API' do
            ...
            put '/mycloud/problems', JSON.generate('solutions' => { 42 => 'do_this', 43 => 'do_that', 44 => nil }),
                'CONTENT_TYPE' => 'application/json'
            expect_redirect_to_queued_task(last_response)

            Models::DeploymentProblem.create(deployment_id: deployment.id, resource_id: 2,
          ...
          describe 'GET', '/:deployment_name/errands' do
            ...
            context 'authenticated access' do
              before do
                ...
                template3 = Models::Template.make(name: 'job_with_bin_run', release: release,
                                                  spec: { templates: { 'foo' => 'bin/run' } })
        ...
        describe 'variables' do
          ...
          it 'returns a unique list of variable ids and names' do
            ...
            expect(last_response.status).to eq(200)
            vars1 = JSON.parse(last_response.body)
            expect(vars1).to match_array([
            ...
            expect(last_response.status).to eq(200)
            vars2 = JSON.parse(last_response.body)
            expect(vars2).to match_array([
                                           { 'id' => 'var_id_1', 'name' => 'var_name_1' },
                                           { 'id' => 'var_id_2', 'name' => 'var_name_2' },
                                           { 'id' => 'var_id_3', 'name' => 'var_name_3' },
          ...
          context 'when deployment does not have variables' do
            ...
            it 'returns an empty array' do
              ...
              expect(last_response.status).to eq(200)
              vars3 = JSON.parse(last_response.body)
              expect(vars3).to be_empty
            end
          end
        end
      end
      ...
      describe 'authorization' do
        ...
        let!(:owned_deployment) do
          Models::Deployment.create_with_teams(name: 'owned_deployment', teams: [dev_team],
                                               manifest: manifest_with_errand('owned_deployment'),
                                               cloud_configs: [cloud_config])
        end
        let!(:other_deployment) do
          Models::Deployment.create_with_teams(name: 'other_deployment', teams: [other_team],
                                               manifest: manifest_with_errand('other_deployment'),
                                               cloud_configs: [cloud_config])
        end
        describe 'when a user has dev team admin membership' do
          before do
            instance = Models::Instance.create(deployment: owned_deployment, job: 'dea', index: 0, state: :started,
                                               uuid: 'F0753566-CA8E-4B28-AD63-7DB3903CD98C',
                                               variable_set: Models::VariableSet.create(deployment: owned_deployment))
            Models::Instance.create(deployment: other_deployment, job: 'dea', index: 0, state: :started,
                                    uuid: '72652FAA-1A9C-4803-8423-BBC3630E49C6',
                                    variable_set: Models::VariableSet.create(deployment: other_deployment))
            Models::Vm.make(agent_id: 'random-id', instance_id: instance.id, active: true)
          end
          ...
          context 'PUT /:deployment/jobs/:job/:index_or_id/resurrection' do
            it 'allows access to owned deployment' do
              expect(put('/owned_deployment/jobs/dea/0/resurrection', '{}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(200)
            end
            it 'denies access to other deployment' do
              expect(put('/other_deployment/jobs/dea/0/resurrection', '{}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(401)
            end
          end

          context 'PUT /:deployment/instance_groups/:instancegroup/:id/ignore' do
            it 'allows access to owned deployment' do
              expect(put('/owned_deployment/instance_groups/dea/F0753566-CA8E-4B28-AD63-7DB3903CD98C/ignore', '{}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(200)
            end
            it 'denies access to other deployment' do
              expect(put('/other_deployment/instance_groups/dea/72652FAA-1A9C-4803-8423-BBC3630E49C6/ignore', '{}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(401)
            end
          end
          ...
          context 'POST /:deployment/properties' do
            it 'allows access to owned deployment' do
              expect(post('/owned_deployment/properties', '{"name": "prop", "value": "bingo"}',
                          'CONTENT_TYPE' => 'application/json').status).to eq(204)
            end

            it 'denies access to other deployment' do
              expect(post('/other_deployment/properties', '{"name": "prop", "value": "bingo"}',
                          'CONTENT_TYPE' => 'application/json').status).to eq(401)
            end
          end
          ...
          context 'PUT /:deployment/properties/:property' do
            before { Models::DeploymentProperty.make(deployment: owned_deployment, name: 'prop', value: 'value') }
            it 'allows access to owned deployment' do
              expect(put('/owned_deployment/properties/prop', '{"value": "bingo"}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(204)
            end

            it 'denies access to other deployment' do
              expect(put('/other_deployment/properties/prop', '{"value": "bingo"}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(401)
            end
          end
          ...
          context 'PUT /:deployment/problems' do
            it 'allows access to owned deployment' do
              expect(put('/owned_deployment/problems', '{"resolutions": {}}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(302)
            end
          ...
          context 'PUT /:deployment/problems' do
            ...
            it 'denies access to other deployment' do
              expect(put('/other_deployment/problems', '{"resolutions": {}}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(401)
            end
          end
          ...
          context 'PUT /:deployment/scan_and_fix' do
            it 'allows access to owned deployment' do
              expect(put('/owned_deployment/scan_and_fix', '{"jobs": []}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(302)
            end

            it 'denies access to other deployment' do
              expect(put('/other_deployment/scan_and_fix', '{"jobs": []}',
                         'CONTENT_TYPE' => 'application/json').status).to eq(401)
            end
          end
          ...
          context 'POST /:deployment/errands/:errand_name/runs' do
            it 'allows access to owned deployment' do
              expect(post('/owned_deployment/errands/errand_job/runs', '{}',
                          'CONTENT_TYPE' => 'application/json').status).to eq(302)
            end

            it 'denies access to other deployment' do
              expect(post('/other_deployment/errands/errand_job/runs', '{}',
                          'CONTENT_TYPE' => 'application/json').status).to eq(401)
            end
          end
