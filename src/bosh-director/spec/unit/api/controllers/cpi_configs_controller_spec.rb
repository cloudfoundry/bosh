  ...
  describe Api::Controllers::CpiConfigsController do
    ...
    describe 'POST', '/' do
      ...
      describe 'when user has readonly access' do
        ...
        it 'denies access' do
          expect(post('/', YAML.dump(Bosh::Spec::NewDeployments.multi_cpi_config), 'CONTENT_TYPE' => 'text/yaml').status)
            .to eq(401)
        end
      end
    end
    ...
    describe 'POST', '/diff' do
      let(:cpi_config) { YAML.dump(Bosh::Spec::NewDeployments.multi_cpi_config) }
