...
module Bosh::Director
  describe DeploymentDeleter do
    ...
    let(:disk_deleter) { instance_double(DiskDeleter) }
    ...
    describe '#delete' do
      ...
      before do
        ...
        allow(disk_deleter).to receive(:delete_dynamic_disks)
      ...
      it 'deletes deployment instances' do
        ...
        deleter.delete(deployment_model, instance_deleter, vm_deleter, disk_deleter)
      end
      ...
      it 'deletes all dynamic disks' do
        expect(disk_deleter).to receive(:delete_dynamic_disks).with(
          deployment_model,
          instance_of(EventLog::Stage),
          {max_threads: 3}
        )
        deleter.delete(deployment_model, instance_deleter, vm_deleter, disk_deleter)
      end
