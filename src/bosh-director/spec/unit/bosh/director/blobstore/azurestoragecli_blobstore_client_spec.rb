  ...
  describe AzurestoragecliBlobstoreClient do
      ...
      context 'when there is no azure-storage-cli' do
        it 'raises an error' do
          ...
          expect { described_class.new(options) }.to raise_error(
            Bosh::Director::Blobstore::BlobstoreError, 'Cannot find azure-storage-cli executable. Please specify azure_storage_cli_path parameter'
          )
        end
      end
    ...
    describe '#delete' do
      it 'should delete an object' do
        ...
        expect(Open3).to receive(:capture3).with('/var/vcap/packages/azure-storage-cli/bin/azure-storage-cli', '-c', expected_config_file.to_s, 'delete', blob_id.to_s)
    ...
    describe '#exists?' do
      it 'should return true if azure-storage-cli reported so' do
        allow(Open3).to receive(:capture3).and_return([nil, nil, success_exit_status])
        expect(Open3).to receive(:capture3).with('/var/vcap/packages/azure-storage-cli/bin/azure-storage-cli', '-c', expected_config_file.to_s, 'exists', blob_id.to_s)
    ...
    describe '#get' do
      ...
      it 'should have correct parameters' do
        allow(Open3).to receive(:capture3).and_return([nil, nil, success_exit_status])
        expect(Open3).to receive(:capture3).with('/var/vcap/packages/azure-storage-cli/bin/azure-storage-cli', '-c', expected_config_file.to_s, 'get', blob_id.to_s, file_path.to_s)
        client.get(blob_id)
      end
    ...
    describe '#sign_url' do
      it 'should return the signed url' do
        expect(Open3).to receive(:capture3)
          .with('/var/vcap/packages/azure-storage-cli/bin/azure-storage-cli', '-c', expected_config_file.to_s, 'sign', blob_id.to_s, 'get', '24h')
