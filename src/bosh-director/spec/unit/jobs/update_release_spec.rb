...
module Bosh::Director
  describe Jobs::UpdateRelease do
    ...
    describe 'process_packages' do
      subject(:update_release) { Jobs::UpdateRelease.new(release_dir) }
      let(:release_dir) { double(:release_dir) }

      before do
        allow(update_release).to receive(:manifest_packages).and_return []
      end

      context 'if it is compiled' do
        before do
          update_release.instance_variable_set(:@compiled_release, true)
        end

        it 'creates compiled packages' do
          expect(update_release).to receive(:create_compiled_packages).with([], release_dir)
          update_release.process_packages(release_dir)
        end
      end

      context 'if it is not compiled' do
        before do
          update_release.instance_variable_set(:@compiled_release, false)
        end
      ...
      context 'if a package fingerprint changes' do
        before do
          ...
          update_release.instance_variable_set(:@release_version_model, release_version_model)
          update_release.instance_variable_set(:@name, 'release-name')
          update_release.instance_variable_set(:@version, 'release-version')
        end
      ...
      context 'if there exists a package that has a matching fingerprint' do
        before do
          ...
          update_release.instance_variable_set(:@release_model, double(id: release_model_id))
          update_release.instance_variable_set(:@release_version_model, release_version_model)
          allow(update_release).to receive(:create_packages)
        end
        ...
        context 'and is not preexisting' do
          context 'and has a nil blobstore id' do
            before do
              # Need to make one of release, name, or version disagree
              update_release.instance_variable_set(:@release_model, double(id: 2))
            end
