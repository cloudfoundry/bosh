...
module Bosh::Director
  describe '20180411165035_migrate_update_strategy_to_vm_strategy.rb' do
    let(:db) { DBSpecHelper.db }
    let(:migration_file) { '20180411165035_migrate_update_strategy_to_vm_strategy.rb' }
    ...
    it 'should update the update strategies to their valid names' do
      ...
      expect(updated_legacy_spec_json['update']['vm_strategy']).to eq('delete-create')
      updated_create_swap_delete_spec_json = JSON.parse(db[:instances].all[1][:spec_json])
      expect(updated_create_swap_delete_spec_json['update']['vm_strategy']).to eq('create-swap-delete')
    end
