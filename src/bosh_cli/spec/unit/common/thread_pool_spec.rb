...
describe Bosh::Cli::ThreadPool do
  ...
  it "should respect max threads" do
    ...
    Bosh::Cli::ThreadPool.new(:max_threads => 2, :logger => @logger).wrap do |pool|
      4.times do
        pool.process do
          lock.synchronize do
            current += 1
  ...
  it "should stop processing new work when there was an exception" do
    ...
    expect {
      Bosh::Cli::ThreadPool.new(:max_threads => 1, :logger => @logger).wrap do |pool|
        10.times do |index|
          pool.process do
