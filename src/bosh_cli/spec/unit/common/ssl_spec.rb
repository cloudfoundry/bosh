require 'spec_helper'
require 'cli/common/ssl'
require 'tmpdir'

describe Bosh::Cli::Ssl::Certificate do
    ...
    context 'when the paths given do not exist' do
      ...
      context 'when the key exists but the certificate does not' do
        ...
        it 'raises an error' do
          ...
          }.to raise_error(Bosh::Cli::Ssl::Certificate::MatchingFileNotFound, /The key that matches the given certificate could not be found\./)
        end
      end
      ...
      context 'when the certificate exists but the key does not' do
        ...
        it 'raises an error' do
          expect {
            server_certificate.load_or_create
          }.to raise_error(Bosh::Cli::Ssl::Certificate::MatchingFileNotFound, /The certificate that matches the given key could not be found\./)
        end
      end
