...
describe Bosh::Cli::Retryable do
  ...
  it 'should raise a RetryCountExceeded error if retries exceeded and block returns false' do
    ...
    }.to raise_error(Bosh::Cli::Common::RetryCountExceeded)
...
describe Bosh::Cli::Retryable::ErrorMatcher do
  ...
  describe 'by_class' do
    ...
    it 'returns an ErrorMatcher that matches any message' do
      expect(Bosh::Cli::Retryable::ErrorMatcher).to receive(:new).with(klass, /.*/).and_call_original
      matcher = Bosh::Cli::Retryable::ErrorMatcher.by_class(klass)
      expect(matcher).to be_an_instance_of(Bosh::Cli::Retryable::ErrorMatcher)
