...
require 'cli/bosh/template/invalid_property_type'
      ...
      describe PropertyHelper do
        ...
        context '#validate_properties_format' do
          ...
          context 'when deployment manifest properties are  NOT valid' do
            ...
            it 'raises an Bosh::Cli::Template::InvalidPropertyType error' do
              key = 'foo.yarb.pow'

              expect {
                @helper.validate_properties_format(props, key)
              }.to raise_error(Bosh::Cli::Template::InvalidPropertyType, "Property 'foo.yarb.pow' expects a hash, but received 'String'")
            end

            context 'when the properties variable is not a hash' do
              it 'raises an Bosh::Cli::Template::InvalidPropertyType if the properties is not a hash' do
                expect {
                  @helper.validate_properties_format(false, 'a')
                }.to raise_error(Bosh::Cli::Template::InvalidPropertyType, "Property 'a' expects a hash, but received 'FalseClass'")
              end
            end
          end
        end
