    ...
    describe '.safe_create' do
      context 'with known provider' do
        ...
        it 'makes retryable client with simple client' do
          wrapped_client = instance_double('Bosh::Cli::Blobstore::SimpleBlobstoreClient')
          expect(SimpleBlobstoreClient)
            .to receive(:new)
            .and_return(wrapped_client)

          sha1_verifiable_client = instance_double('Bosh::Cli::Blobstore::Sha1VerifiableBlobstoreClient')
          expect(Sha1VerifiableBlobstoreClient)
            .to receive(:new)
            .with(wrapped_client)
            .and_return(sha1_verifiable_client)

          retryable = instance_double('Bosh::Cli::Retryable')
          expect(Bosh::Cli::Retryable)
            .to receive(:new)
            .and_return(retryable)

          retryable_client = instance_double('Bosh::Cli::Blobstore::RetryableBlobstoreClient')
