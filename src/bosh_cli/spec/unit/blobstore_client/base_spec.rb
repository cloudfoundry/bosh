...
describe Bosh::Cli::Blobstore::BaseClient do
  ...
  describe '#create' do
    it 'should raise a NotImplemented exception' do
      expect { subject.create('contents') }.to raise_error(
        Bosh::Cli::Blobstore::NotImplemented, 'not supported by this blobstore')
    end

    it 'should raise BlobstoreError exceptions' do
      expect(subject).to receive(:create_file).and_raise(
        Bosh::Cli::Blobstore::BlobstoreError, 'Could not create object')
    ...
    it 'should trap generic exceptions and raise a BlobstoreError exception' do
      ...
      expect { subject.create('contents') }.to raise_error(
        Bosh::Cli::Blobstore::BlobstoreError,
  ...
  describe '#get' do
    it 'allows to pass options optionally' do
      expect { subject.get('id', 'file')     }.to raise_error(Bosh::Cli::Blobstore::NotImplemented)
      expect { subject.get('id', 'file', {}) }.to raise_error(Bosh::Cli::Blobstore::NotImplemented)
    end
    ...
    it 'should raise BlobstoreError exceptions' do
      expect(subject).to receive(:get_file).and_raise(
        Bosh::Cli::Blobstore::BlobstoreError, 'Could not fetch object')
