...
describe Bosh::Cli::Blobstore::DavBlobstoreClient, nginx: true do
  ...
  context 'read users' do
    ...
    context 'with unauthorized user' do
      ...
      it "doesn't allow read" do
        expect { subject.get('test') }.to raise_error(Bosh::Cli::Blobstore::BlobstoreError, /Could not fetch object, 401/)
      end

      it 'does not allow checking for existance' do
        expect { subject.exists?('test') }.to raise_error(Bosh::Cli::Blobstore::BlobstoreError, /Could not get object existence, 401/)
      end
    end
  ...
  context 'write users' do
    context 'with authorized user' do
     ...
     it 'should raise NotFound error when deleting non-existing file' do
       expect { subject.delete('non-exist-file') }.to raise_error Bosh::Cli::Blobstore::NotFound, /Object 'non-exist-file' is not found/
     end
    ...
    context 'with unauthorized user' do
      ...
      it "doesn't allow write" do
        expect { subject.create('foo', 'test') }.to raise_error(Bosh::Cli::Blobstore::BlobstoreError, /Could not create object, 401/)
      end

      it "doesn't allow delete" do
        expect { subject.delete('test') }.to raise_error(Bosh::Cli::Blobstore::BlobstoreError, /Could not delete object, 401/)
      end
