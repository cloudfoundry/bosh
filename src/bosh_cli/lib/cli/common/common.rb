...
require 'cli/common/errors'
require 'cli/common/retryable'
...
module Bosh
  module Cli
    ...
    module Common
      ...
      # Examples:
      #   Bosh::Cli::Common.retryable do |retries, exception|
      #     puts "try #{retries} failed with exception: #{exception}" if retries > 0
      #     pick_up_soap
      #   end
      #
      #   # Wait for EC2 instance to be terminated
      #   Bosh::Cli::Common.retryable(on: AWS::EC2::Errors::RequestLimitExceeded) do |retries, exception|
      ...
      #  end
      #
      #  Bosh::Cli::Common.retryable(ensure: ensure_cb) do
      #    # process file
      #  end
      #
      ...
      #   Example:
      #   Bosh::Cli::Common.retryable(matching: /IO timeout/) do |retries, exception|
      ...
      #   Example:
      #   Bosh::Cli::Common.retryable(on: [StandardError, ArgumentError]) do
      ...
      #   Example:
      #   Bosh::Cli::Common.retryable(sleep: lambda { |n,e| logger.info(e.message) if e; 4**n }) { }
      #
      # @options opts [Fixnum] :tries
      #   Default: 2
      #   Number of times to try
      #   Example:
      #   Bosh::Cli::Common.retryable(tries: 3, on: OpenURI::HTTPError) do
      #     xml = open("http://example.com/test.xml").read
      #   end
      #
      def retryable(options = {}, &block)
        Bosh::Cli::Retryable.new(options).retryer(&block)
      end
