...
run_in_chroot $chroot "yum -c /custom_photonos_yum.conf update --assumeyes"
run_in_chroot $chroot "yum -c /custom_photonos_yum.conf --verbose --assumeyes install photon-release"
run_in_chroot $chroot "yum -c /custom_photonos_yum.conf --verbose --assumeyes install linux-api-headers glibc glibc-devel glibc-lang zlib zlib-devel file binutils binutils-devel gmp gmp-devel mpfr mpfr-devel mpc coreutils flex bison bindutils sudo e2fsprogs elfutils shadow cracklib Linux-PAM findutils diffutils sed grep tar gawk which make patch gzip openssl openssh wget vim tdnf yum curl grub2 tzdata readline-devel ncurses-devel cmake bzip2-devel cdrkit ruby logrotate ntp"
run_in_chroot $chroot "yum -c /custom_photonos_yum.conf --verbose --assumeyes install linux dracut dkms linux-dev"
run_in_chroot $chroot "yum -c /custom_photonos_yum.conf --verbose --assumeyes install systemd rsyslog cronie gcc kpartx NetworkManager pkg-config ncurses bash bzip2 cracklib-dicts shadow procps-ng iana-etc readline coreutils bc libtool inetutils findutils xz iproute2 util-linux ca-certificates iptables attr libcap expat dbus sqlite-autoconf nspr nss rpm libffi gdbm python2 python2-libs pcre glib libxml2 photon-release cpio gzip db libsolv libgpg-error hawkey libassuan gpgme librepo tdnf libdnet xerces-c xml-security-c libmspack  krb5 e2fsprogs-devel kmod dhcp-client initscripts libtirpc lsof runit"

run_in_chroot $chroot "yum -c /custom_photonos_yum.conf clean all"
