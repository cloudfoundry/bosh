...
function createCertWithCA () { name=$1
  ...
  openssl req -nodes -new -newkey rsa:1024 -out ${name}.csr -keyout ${name}.key -subj '/C=US/O=BOSH/CN=127.0.0.1/subjectAltName=DNS.1=127.0.0.1,DNS.2=0.0.0.0'
...
function createSelfSignedCert () { name=$1
  ...
  openssl req -nodes -new -newkey rsa:1024 -out ${name}.csr -keyout ${name}.key -subj '/C=US/O=Pivotal/CN=127.0.0.1/subjectAltName=DNS.1=127.0.0.1,DNS.2=0.0.0.0'
