...
namespace :fly do
  ...
  task :integration do
    execute('test-integration', '-p --inputs-from bosh:262.x/integration-2.3-postgres', {
  ...
  task :integration_gocli do
    execute('test-integration-gocli', '-p --inputs-from bosh:262.x/integration-postgres-gocli-sha2', {
        DB: (ENV['DB'] || 'postgresql'), SPEC_PATH: (ENV['SPEC_PATH'] || nil)
    })
  end
