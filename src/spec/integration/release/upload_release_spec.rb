...
describe 'upload release', type: :integration do
  ...
  it 'sparsely uploads the release' do
    Dir.chdir(ClientSandbox.test_release_dir) do
      ...
      expect(out).to match /Started processing 25 existing jobs > Processing 25 existing jobs.*Done/
  ...
  describe 'when the release is remote' do
    ...
    it 'accepts the release when the sha1 matches' do
      ...
      expect(output).to include("Started creating new packages")
      expect(output).to include("Started creating new jobs")
      expect(output).to include("Release uploaded")
    end
    ...
    it 'rejects the release when the sha1 does not match' do
      ...
      }.to raise_error(RuntimeError, /Error 30015: Expected stream to have digest 'abcd1234' but was '#{sha1}'/)
    end
  end
  ...
  describe 'uploading release with --fix' do
    ...
    context 'when uploading compiled package' do
      it 'Re-uploads all compiled packages to replace old ones' do
        ...
        output = bosh_runner.run("upload release #{spec_asset('release-hello-go-50-on-toronto-os-stemcell-1.tgz')} --fix")
        expect(output).to include('Release uploaded')
