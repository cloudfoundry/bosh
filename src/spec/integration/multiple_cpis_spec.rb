...
describe 'Using multiple CPIs', type: :integration do
  ...
  let(:cpi_config) do
    Bosh::Spec::Deployments.multi_cpi_config(current_sandbox.sandbox_path(Bosh::Dev::Sandbox::Main::EXTERNAL_CPI))
  end

  let(:instance_group) { Bosh::Spec::Deployments.simple_instance_group(azs: %w[z1 z2]) }
  let(:deployment) { Bosh::Spec::Deployments.test_release_manifest_with_stemcell.merge('instance_groups' => [instance_group]) }
  ...
  context 'when VM is deployed to az that has been removed from cloud config' do
    it 'falls back to existing CPIs and succeeds in deleting' do
      ...
      instance_group = Bosh::Spec::Deployments.simple_instance_group(azs: ['z1'])
