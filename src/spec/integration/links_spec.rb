  ...
  - Multiple instance groups provide links of type 'db'. Cannot decide which one to use for instance group 'optional_db'.
    ...
    context 'when exporting a release with templates that have links' do
      ...
      it 'should successfully compile a release without complaining about missing links' do
        ...
        expect(out).to match(/Started compiling packages > pkg_2\/[a-f0-9]+. Done/)
        expect(out).to match(/Started compiling packages > pkg_3_depends_on_2\/[a-f0-9]+. Done/)

        expect(out).to match(/Started copying packages/)
        expect(out).to match(/Started copying packages > pkg_1\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying packages > pkg_2\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying packages > pkg_3_depends_on_2\/[a-f0-9]+. Done/)

        expect(out).to match(/Started copying jobs/)
        expect(out).to match(/Started copying jobs > addon\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > api_server\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > api_server_with_bad_link_types\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > api_server_with_bad_optional_links\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > api_server_with_optional_db_link\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > api_server_with_optional_links_1\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > api_server_with_optional_links_2\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > backup_database\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > database\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > mongo_db\/[a-f0-9]+. Done/)
        expect(out).to match(/Started copying jobs > node\/[a-f0-9]+. Done/)
