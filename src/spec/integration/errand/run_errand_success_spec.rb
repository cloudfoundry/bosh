...
describe 'run-errand success', type: :integration, with_tmp_dir: true do
  ...
  context 'while errand is running' do
    ...
    it 'creates a deployment lock' do
      deploy_from_scratch(manifest_hash: manifest_hash_errand, cloud_config_hash: Bosh::Spec::DeploymentManifestHelper.simple_cloud_config)
  ...
  context 'when manifest file is greater than 64Kb' do
    ...
    let(:manifest_hash) do
      ...
      manifest.merge(Bosh::Spec::DeploymentManifestHelper.manifest_with_errand)
    end
  ...
  context 'when configured with addons' do
    ...
    let(:runtime_config_hash) do
      config_hash = Bosh::Spec::DeploymentManifestHelper.runtime_config_with_addon
    ...
    let(:manifest_with_errand) do
      errand = Bosh::Spec::DeploymentManifestHelper.manifest_with_errand
  ...
  context 'errand name != first job' do
    let(:manifest_hash) do
      manifest_hash = Bosh::Spec::DeploymentManifestHelper.manifest_with_errand_on_service_instance
  ...
  context 'when instance lifecycle = service' do
    ...
    let(:manifest_hash) do
      Bosh::Spec::DeploymentManifestHelper.simple_manifest_with_instance_groups.merge(
        'instance_groups' => [
          {
  ...
  context 'when running an errand across multiple instances' do
    ...
    let(:manifest_hash) do
      ...
      manifest = Bosh::Spec::DeploymentManifestHelper.manifest_with_errand_on_service_instance
