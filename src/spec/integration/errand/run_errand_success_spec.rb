...
describe 'run-errand success', type: :integration, with_tmp_dir: true do
  ...
  context 'when manifest file is greater than 64Kb' do
    ...
    let(:manifest_hash) do
      ...
      manifest.merge(SharedSupport::DeploymentManifestHelper.manifest_with_errand)
    end
  ...
  context 'when configured with addons' do
    ...
    let(:runtime_config_hash) do
      config_hash = SharedSupport::DeploymentManifestHelper.runtime_config_with_addon
    ...
    let(:manifest_with_errand) do
      errand = SharedSupport::DeploymentManifestHelper.manifest_with_errand
  ...
  context 'errand name != first job' do
    let(:manifest_hash) do
      manifest_hash = SharedSupport::DeploymentManifestHelper.manifest_with_errand_on_service_instance
