...
describe 'cli: variables', type: :integration do
  ...
  def assert_count_variable_id_and_name(count_variable_ids, count_variable_names)
    ...
    variable_ids = variables.map do |obj|
      obj['id']
    end
    expect(variable_ids.uniq.length).to eq(count_variable_ids)

    variable_names = variables.map do |obj|
      obj['name']
    end
  ...
  context 'when you have unused variable sets' do
    ...
    context 'when you have failed deploys' do
      ...
      before do
        ...
        deploy_with_settings(cloud_config, client_env, manifest_hash)
      ...
      def deploy_with_settings(cloud_config, client_env, manifest_hash)
        deploy_from_scratch(
          cloud_config_hash: cloud_config,
          env: client_env,
