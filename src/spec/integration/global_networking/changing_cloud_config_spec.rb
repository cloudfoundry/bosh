...
describe 'Changing cloud config', type: :integration do
  ...
  describe 'changing the cloud config while deploying' do
    it 'should continue to use the original cloud config when deploying a job' do
      ...
      deployment_manifest = SharedSupport::DeploymentManifestHelper.deployment_manifest(instances: 1, job: 'foobar_without_packages')
  ...
  describe 'changing the cloud config with health monitor running', hm: true do
    ...
    it 'resurrects vm with original cloud config and IP' do
      ...
      expect(original_instance.ips).to eq(resurrected_instance.ips),
                                       "Original vm IPs '#{original_instance.ips}' do not match resurrected vm IPs " \
                                       "'#{resurrected_instance.ips}', original output: #{original_vms_output}," \
                                       " resurrected output: #{resurrected_vms_output}"
    end
  end
  ...
  def upload_a_different_cloud_config
    new_cloud_config = SharedSupport::DeploymentManifestHelper.cloud_config_with_subnet(available_ips: 0)
