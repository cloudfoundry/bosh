...
describe 'multiple persistent disks', type: :integration do
  ...
  context 'initial deployment state', type: :integration do
    with_reset_sandbox_before_all(dummy_cpi_api_version: 2)
    ...
    before(:all) do
      ...
      manifest = {
        ...
        'instance_groups' => [{
          ...
          'networks' => [{'name' => 'a'}],
          'persistent_disks' => [
            {'type' => 'low-performance-disk-type', 'name' => 'low-iops-persistent-disk-name'},
            {'type' => 'high-performance-disk-type', 'name' => 'high-iops-persistent-disk-name'}
          ]
        }],
  ...
  context 'state-modifying operations', type: :integration do
