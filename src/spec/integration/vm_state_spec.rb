...
describe 'vm state', type: :integration do
  ...
  describe 'detached' do
    ...
    it 'releases previously reserved IP when state changed with new static IP' do
      ...
      deploy_from_scratch(manifest_hash: first_manifest_hash, cloud_config_hash: Bosh::Spec::Deployments.simple_cloud_config)
  ...
  context 'instances have gaps in indexes' do
    ...
    it 'it keeps instances with left static IP and deletes instances with removed IPs' do
      ...
      output = bosh_runner.run('restart foobar/1', deployment_name: deployment_name)
      expect(output).to match(%r{foobar/[a-z0-9\-]+ \(1\)})
      expect(output).to_not match(%r{foobar/[a-z0-9\-]+ \(2\)})

      output = bosh_runner.run('restart foobar/2', deployment_name: deployment_name)
      expect(output).to_not match(%r{foobar/[a-z0-9\-]+ \(1\)})
      expect(output).to match(%r{foobar/[a-z0-9\-]+ \(2\)})
    end
  end
