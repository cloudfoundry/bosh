...
describe 'resurrector', type: :integration, hm: true do
  ...
  it 'resurrects vms based on resurrection config' do
    ...
    deployment_hash_disabled['instance_groups'][1] =
    ...
    original_instance_enabled = director.instance('foobar', '0', deployment_name: 'simple_enabled')
    resurrected_instance = director.kill_vm_and_wait_for_resurrection(original_instance_enabled, deployment_name: 'simple_enabled')
    expect(resurrected_instance.vm_cid).to_not eq(original_instance_enabled.vm_cid)
    ...
    original_instance_enabled = director.find_instance(instances, 'foobar_without_packages', '0')
    disabled_instance = director.find_instance(instances, 'foobar', '0')

    resurrected_instance = director.kill_vm_and_wait_for_resurrection(original_instance_enabled, deployment_name: 'simple_disabled')
    expect(resurrected_instance.vm_cid).to_not eq(original_instance_enabled.vm_cid)
