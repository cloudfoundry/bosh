    ...
    context 'when the provider is updated' do
      ...
      context 'and the consumer is redeployed' do
        it 'should fail to resolve the link' do
          expect { deploy_simple_manifest(manifest_hash: second_manifest) }.to
          raise_error(RuntimeError, Regexp.new("Failed to resolve links from deployment 'second'. See errors below:\n" \
