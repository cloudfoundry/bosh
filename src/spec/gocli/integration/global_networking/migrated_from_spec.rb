...
describe 'migrated from', type: :integration do
  ...
  xcontext 'when migrating to availability zones' do
    ...
    def migrate_legacy_etcd_z1_and_z2
      ...
      expect(original_instances.map(&:instance_group_name)).to match_array(['etcd_z1', 'etcd_z2'])
    ...
    context 'when using same static reservation' do
      ...
      it 'keeps VM, disk and IPs and updates AZs' do
        ...
        expect(new_instances.map(&:instance_group_name)).to eq(['etcd', 'etcd'])
    ...
    context 'when the number of original instances is greater than the number of new instances' do
      ...
      it 'deletes extra instances' do
        ...
        expect(original_instances.map(&:instance_group_name)).to match_array(['etcd_z1', 'etcd_z1', 'etcd_z2'])
    ...
    context 'when migrating without persistent disks' do
      ...
      it 'balances vms across azs' do
        ...
        expect(original_instances.map(&:instance_group_name)).to match_array(['etcd_z1', 'etcd_z1'])
    ...
    context 'when deployment fails' do
      it 'job instances should have migrated names, indexes, bootstrap and az' do
        ...
        new_instances = director.instances
        etcd_instance_1 = new_instances.find { |vm| vm.instance_group_name == 'etcd' && vm.index == '0' }
        ...
        etcd_instance_2 = new_instances.find { |vm| vm.instance_group_name == 'etcd' && vm.index == '1' }
