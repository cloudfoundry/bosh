...
6: Delete VM reference (forceful; may need to manually delete VM from the Cloud to avoid IP conflicts)"))
      ...
      context 'when there is an ignored vm' do
        before do
          instance_to_ignore =director.instances.select{|instance| instance.instance_group_name == 'foobar' && instance.index == '1'}.first
...
2: Delete disk reference (DANGEROUS!)'))
      ...
      it 'deletes disk reference when delete_disk_reference is set and passes correct stemcell_api_version in the context of the CPI call' do
