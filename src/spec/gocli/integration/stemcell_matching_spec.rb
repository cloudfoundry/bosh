...
describe 'stemcell matches api', type: :integration do
  ...
  it 'indicates when stemcells have not been presented to configured cpis' do
    ...
    resp = send_director_post_request('/stemcell_matches', '', JSON.dump([{'name' => 'ubuntu-stemcell', 'version' => '1'}]))
    expect(resp.code).to eq("200")
    expect(resp.body).to eq('{"missing":[{"name":"ubuntu-stemcell","version":"1"}]}')

    bosh_runner.run("upload-stemcell #{stemcell_filename}")

    resp = send_director_post_request('/stemcell_matches', '', JSON.dump([{'name' => 'ubuntu-stemcell', 'version' => '1'}]))
    expect(resp.code).to eq("200")
    expect(resp.body).to eq('{"missing":[]}')
