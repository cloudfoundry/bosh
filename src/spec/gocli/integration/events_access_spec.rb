...
describe 'events endpoint access', type: :integration do
  ...
  before do
    ...
    deploy_from_scratch(manifest_hash: deployment_hash, client: director_client_env['BOSH_CLIENT'], client_secret: director_client_env['BOSH_CLIENT_SECRET'])
  end
  ...
  it 'bosh.teams.X.read should be able to see all events in the director' do
    ...
    expect(output).to include '34 events'
  end
  ...
  it 'no-access should not be able to see all events in the director' do
    ...
    expect(output).to_not include '34 events'
  end
end
