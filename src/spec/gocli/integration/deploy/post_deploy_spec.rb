...
describe 'post deploy scripts', type: :integration do
  ...
  context 'when running post-deploy scripts is not supported' do
    ...
    it 'runs the post-deploy scripts and redirects stdout/stderr to post-deploy.stdout.log/post-deploy.stderr.log for each job' do
      deploy(manifest_hash: manifest)
      ...
      expect(File.exist?(File.join(log_path, 'job_1_with_post_deploy_script/post-deploy.stdout.log'))).to be_falsey
      expect(File.exist?(File.join(log_path, 'job_1_with_post_deploy_script/post-deploy.stderr.log'))).to be_falsey
      expect(File.exist?(File.join(log_path, 'job_2_with_post_deploy_script/post-deploy.stdout.log'))).to be_falsey
    end
  end
end
