...
describe 'Links', type: :integration do
  ...
  let(:shared_manifest_with_errand) do
    Bosh::Spec::NewDeployments.simple_manifest_with_instance_groups.tap do |manifest|
      manifest['name'] = 'shared_deployment_with_errand'
      manifest['instance_groups'] = [shared_provider_instance_group, shared_consumer_instance_group,
                                     errand_consumer_instance_group]
    end
  end
  ...
  let(:colocated_errand_instance_group) do
    Bosh::Spec::NewDeployments.simple_instance_group(
      name: 'errand_ig',
      jobs: [{ 'name' => 'database'}, { 'name' => 'errand_with_links' }],
      instances: 1,
      azs: ['z1'],
      )
  end

  let(:colocated_errand_manifest) do
    Bosh::Spec::NewDeployments.simple_manifest_with_instance_groups.tap do |manifest|
      manifest['name'] = 'colocated_errand_deployment'
      manifest['instance_groups'] = [colocated_errand_instance_group]
    end
  end
  ...
  it 'sets up initial state' do
    ...
    puts 'Colocated Errand Deployment'
    deploy_simple_manifest(manifest_hash: colocated_errand_manifest)
    new_instance = director.instance('errand_ig', '0', deployment_name: 'colocated_errand_deployment')
    puts YAML.safe_load(new_instance.read_job_template('errand_with_links', 'config.yml'))

    puts 'Shared Deployment with Errand'
    deploy_simple_manifest(manifest_hash: shared_manifest_with_errand)
    new_instance = director.instance('shared_consumer_ig', '0', deployment_name: 'shared_deployment_with_errand')
    puts YAML.safe_load(new_instance.read_job_template('api_server', 'config.yml'))
    ...
    bosh_runner.run('-d colocated_errand_deployment stop --hard')
    bosh_runner.run('-d shared_deployment_with_errand stop --hard')
    puts 'All good!'
