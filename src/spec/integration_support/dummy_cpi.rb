...
class DummyCpi
  ...
  def create_vm(agent_id, stemcell_id, cloud_properties, networks, disk_cids, env)
    ...
    raise Bosh::Clouds::CloudError.new('Creating vm failed') if cmd.failed?
  ...
  def kill_agents
    vm_cids.each { |agent_pid| kill_process(agent_pid) }
  end
  ...
  def agent_cmd(agent_id, legacy_agent_path)
    go_agent_exe = if legacy_agent_path.nil?
                     File.join(BOSH_REPO_ROOT, 'src', 'tmp', 'bin', 'bosh-agent')
                   else
                     legacy_agent_path
                   end
  ...
  class InputsRecorder
    ...
    def read_all
      return [] unless File.exist?(ordered_file_path)

      content = File.read(ordered_file_path)
      @logger.debug("Reading all inputs: #{content}")
      content.split("\n").map do |request|
        data = JSON.parse(request)
        CpiInvocation.new(data['method_name'], data['inputs'], data['context'])
      end
    end
