...
module IntegrationSupport
  ...
  class Director
    ...
    def instance(instance_group_name, index_or_id, options = { deployment_name: SharedSupport::DeploymentManifestHelper::DEFAULT_DEPLOYMENT_NAME })
    ...
    def find_instance(instances, instance_group_name, index_or_id)
      instance = instances.detect { |instance| instance.instance_group_name == instance_group_name && (instance.index == index_or_id || instance.id == index_or_id)}
    ...
    def wait_for_vm(instance_group_name, index, timeout_seconds, options = {deployment_name: SharedSupport::DeploymentManifestHelper::DEFAULT_DEPLOYMENT_NAME})
      start_time = Time.now
      loop do
    ...
    def kill_vm_and_wait_for_resurrection(vm, options={deployment_name: SharedSupport::DeploymentManifestHelper::DEFAULT_DEPLOYMENT_NAME})
      vm.kill_agent
