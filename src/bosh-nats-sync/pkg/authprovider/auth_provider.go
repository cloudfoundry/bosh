...
func (a *AuthProvider) uaaTokenHeader(ctx context.Context, uaaURL string) (string, error) {
	...
	// verify the JWT RSA signature. This guards against a compromised token
	// even when the TLS channel is intact.
	if pubKey := strings.TrimSpace(a.config.UAAPublicKey); pubKey != "" {
		if err := verifyJWTRSASignature(tok.AccessToken, pubKey); err != nil {
...
// rsaJWTAlgorithms maps the RSASSA-PKCS1-v1.5 JWT "alg" values UAA can sign
// with to their corresponding hash. CF::UAA::TokenCoder.decode (Ruby) accepts
// any algorithm JWT::VERIFY_METHODS supports; UAA itself only ever signs with
// RS256, RS384, or RS512, so those are the ones worth supporting here.
var rsaJWTAlgorithms = map[string]crypto.Hash{
	"RS256": crypto.SHA256,
	"RS384": crypto.SHA384,
	"RS512": crypto.SHA512,
}

// verifyJWTRSASignature verifies that the JWT was signed with the RSA private
// key corresponding to pemPublicKey. It mirrors the Ruby UAAToken
// decode_options path where { pkey: @uaa_public_key, verify: true } is passed
// to CF::UAA::TokenCoder.decode when a public key is configured.
func verifyJWTRSASignature(jwtToken, pemPublicKey string) error {
	...
	hash, ok := rsaJWTAlgorithms[header.Alg]
	if !ok {
		return fmt.Errorf("unsupported JWT algorithm %q: only RS256, RS384, RS512 are supported", header.Alg)
	}
