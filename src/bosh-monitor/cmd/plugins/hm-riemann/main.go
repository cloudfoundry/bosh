...
// severityName mirrors the Ruby SEVERITY_MAP in Events::Alert.
...
func processAlert(addr string, event *pluginlib.EventData, cmds chan<- *pluginlib.Command) {
	state := severityName[event.Severity]
	if state == "" {
...
func sendToRiemann(addr string, payload map[string]interface{}, cmds chan<- *pluginlib.Command) {
	...
	if _, err := conn.Write(data); err != nil {
		cmds <- pluginlib.LogCommand("error", fmt.Sprintf("Error writing riemann event: %v", err))
	}
}
