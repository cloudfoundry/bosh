...
// ── JWT RS256 verification (mirrors Ruby UAAToken decode_options) ─────────────

var _ = Describe("AuthProvider — uaa_public_key JWT verification", func() {
	// makeUAAAuthInfo returns a user_authentication block pointing at the given
	// server URL.
	makeUAAAuthInfo := func(url string) map[string]interface{} {
		return map[string]interface{}{
			"user_authentication": map[string]interface{}{
				"type": "uaa",
				"options": map[string]interface{}{
	...
	// Ruby: when uaa_public_key is set → { pkey: @uaa_public_key, verify: true }
	It("accepts a token whose RS256 signature matches uaa_public_key", func() {
		privKey, pubKeyPEM := generateRSAKeyPair()
		validJWT := createRS256JWT(privKey, time.Now().Add(time.Hour))
		...
		ap := director.NewAuthProvider(makeUAAAuthInfo(uaaServer.URL), director.Config{
			ClientID:     "fake-client",
			ClientSecret: "fake-secret",
	...
	// Ruby: when uaa_public_key is set but signature doesn't match → error
	It("returns empty and logs an error when the token is signed with a different key", func() {
		_, pubKeyPEM := generateRSAKeyPair()
		otherKey, _ := generateRSAKeyPair()
		wrongJWT := createRS256JWT(otherKey, time.Now().Add(time.Hour))

		var logBuf bytes.Buffer
		errLogger := slog.New(slog.NewTextHandler(&logBuf, &slog.HandlerOptions{Level: slog.LevelDebug}))
	...
	// Ruby: when uaa_public_key is empty → { verify: false }
	It("skips JWT verification and accepts opaque tokens when uaa_public_key is empty", func() {
		uaaServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			// An opaque (non-JWT) token — verification must be skipped.
