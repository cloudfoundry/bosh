...
describe Bosh::Monitor::Plugins::Pagerduty do
  ...
  it 'sends events to Pagerduty' do
    ...
    Sync do
      ...
      expect(@plugin).to receive(:send_http_post_request_synchronous).with(uri: uri, request: alert_request)
      expect(@plugin).to receive(:send_http_post_request_synchronous).with(uri: uri, request: heartbeat_request)
