...
describe 'Bhm::Director' do
  ...
  context 'when director is running in non-UAA mode' do
    ...
    it 'can fetch resurrection config from BOSH director' do
      ...
      with_fiber do
        expect(director.get_resurrection_config).to eq(resurrection_config)
      end
    end
    ...
    it 'raises an error if resurrection config cannot be fetched' do
      ...
      with_fiber do
        expect {
          director.get_resurrection_config
        }.to raise_error(Bhm::DirectorError, 'Cannot get resurrection config from director at http://localhost:8080/director/configs?type=resurrection&latest=true: 500 foo')
      end
    end
