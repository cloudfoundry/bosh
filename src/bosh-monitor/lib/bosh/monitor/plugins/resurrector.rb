  ...
  module Plugins
    class Resurrector < Base
      ...
      def process(alert)
        ...
        if state.meltdown?
          ...
          unless jobs_to_instances_resurrection_enabled.empty?
            ...
            send_http_put_request(uri: url.to_s, request: request, ca_cert_path: director_ca_cert)
          end
      ...
      def scan_and_fix_already_queued_or_processing?(deployment_name)
        ...
        body, status = send_http_get_request_synchronous(uri:url.to_s, headers: headers, ca_cert_path:director_ca_cert)
      ...
      def director_info
        ...
        url = @uri.dup
        url.path = '/info'
        body, status = send_http_get_request_synchronous(uri: url.to_s, ca_cert_path: director_ca_cert)
