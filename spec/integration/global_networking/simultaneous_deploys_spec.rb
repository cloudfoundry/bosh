...
describe 'simultaneous deploys', type: :integration do
  ...
  describe 'running errand during deploy' do
    ...
    it 'raise correct error message when we do not have enough IPs for the errand and the deploy' do
      cloud_config = Bosh::Spec::NetworkingManifest.cloud_config(available_ips: 2)
  ...
  describe 'running two errands' do
    ...
    def run_errand_in_thread(errand_manifest, errand_runner)
      ...
      errand_thread = Thread.new do
        deployment_manifest = yaml_file(errand_manifest['name'], errand_manifest)
        errand_runner.run("deployment #{deployment_manifest.path}")
        output, exit_code = errand_runner.run("run errand errand_job", return_exit_code: true, failure_expected: true)
        errand_result.merge!(
          output: output,
          exit_code: exit_code
        )
