...
require "digest/sha1"
require "tmpdir"
require "../director/lib/director/version"
...
describe Bosh::Spec::IntegrationTest do
  ...
  def director_version
    "v#{Bosh::Director::VERSION} (#{`(git show-ref --head --hash=8 2> /dev/null || echo 00000000) | head -n1`.strip})"
  end
  ...
  it "keeps track of user associated with target" do
    ...
    expect_output("--skip-director-checks status", <<-OUT)
        Target         Unknown Director (http://bar) vn/a
        UUID           n/a
        User           jane
        Deployment     not set
    OUT
    ...
    expect_output("--skip-director-checks status", <<-OUT)
        Target         Unknown Director (http://foo) vn/a
        UUID           n/a
        User           john
        Deployment     not set
    OUT
