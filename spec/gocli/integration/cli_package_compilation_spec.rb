...
describe 'cli: package compilation', type: :integration do
  ...
  it 'uses compile package cache for previously compiled packages' do
    ...
    deployment_manifest = yaml_file('deployment_manifest', manifest_hash)
    ...
    bosh_runner.run("-d #{deployment_name} deploy #{deployment_manifest.path}")
    ...
    bosh_runner.run("-d #{deployment_name} delete-deployment")
