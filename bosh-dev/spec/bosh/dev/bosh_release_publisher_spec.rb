...
describe Bosh::Dev::BoshReleasePublisher do
  ...
  describe '.setup_for' do
    it 'instantiates a publisher with the given build and upload/download adapters' do
      Bosh::Dev::DownloadAdapter.stub(:new).with(kind_of(Logger)).and_return(download_adapter)
      Bosh::Dev::UploadAdapter.stub(:new).and_return(upload_adapter)

      publisher = double
      described_class.should_receive(:new).with(candidate_build, upload_adapter, download_adapter).and_return(publisher)
  ...
  describe '#publish' do
    it 'uploads the build and then stages the changes' do
      ...
      publisher = described_class.new(candidate_build, upload_adapter, download_adapter)
