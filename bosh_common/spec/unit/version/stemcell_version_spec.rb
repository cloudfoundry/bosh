...
module Bosh::Common::Version
  describe StemcellVersion do
    ...
    describe '#matches_patch_version' do
      context 'using the static helper method' do
        it 'calls the instance method' do
          ...
          expect(fake_version_a).to receive(:matches_patch_version).with(fake_version_b)
          described_class.match_patch_versions('1.0', '1.1')
        end
      end
      ...
      context 'when stemcell uses semi-semantic versioning' do
        ...
        it 'ignores all non-release segments' do
          ...
          expect(version.matches_patch_version(version_with_pre_release)).to be(true)
          expect(version.matches_patch_version(version_with_post_release)).to be(true)
          expect(version_with_pre_release.matches_patch_version(version_with_post_release)).to be(true)
        end
      end
      ...
      context 'when stemcell uses semantic versioning' do
        ...
        context 'comparing to other minor versions' do
          it 'does not match a newer minor version' do
            newer_minor_version = described_class.parse('1.2.0')
            expect(version.matches_patch_version(newer_minor_version)).to be(false)
          end

          it 'does not match an older minor version' do
            older_minor_version = described_class.parse('1.0.9')
            expect(version.matches_patch_version(older_minor_version)).to be(false)
          end
        end
        ...
        context 'comparing to other major versions' do
          it 'does not match a newer major version' do
            ...
            expect(version.matches_patch_version(newer_major_version)).to be(false)
          end

          it 'does not match an older major version' do
            older_major_version = described_class.parse('0.9.9')
            expect(version.matches_patch_version(older_major_version)).to be(false)
          end
        end
      end
    end
