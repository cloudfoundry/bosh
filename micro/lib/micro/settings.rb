...
module VCAP
  module Micro
    class Settings
      ...
      def self.preserve_secret(service, key)
        @preserve[service][key] if @preserve[service]
      end
      ...
      def self.set_prop(properties, service, key, len)
        ...
        properties[service][key] = secret(len) unless properties[service][key] and preserve_secret(service, key)
      end
