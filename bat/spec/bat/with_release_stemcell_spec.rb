...
describe 'with release and stemcell and two deployments' do
  ...
  context 'deployment without static ip' do
    ...
    xit 'should update a job with multiple instances in parallel' do # Uncomment the before and afters when fixing this test
      ...
      times['batlight/2']['started'].should be >= [times['batlight/0']['finished'], times['batlight/1']['finished']].min
    end
  end
  ...
  context 'first deployment' do
    ...
    it 'should use job colocation', ssh: true do
      @jobs.each do |job|
        ssh(static_ip, 'vcap', "pgrep -lf #{job}", @our_ssh_options).should match %r{#{job}}
      end
    end
