...
describe Bosh::Director::PackageCompiler do
  ...
  describe "compile" do
    ...
    it "should compile a package if it's not already compiled" do
      ...
      check_event_log do |events|
        ...
        events[0].should == {
          ...
          "task" => "test_pkg/33", "index" => 1, "total" => 1, "state" => "started", "progress" => 0, "ticker_data"=>""
        }
        events[1].should == {
          "time" => anything, "stage" => "Compiling packages", "tags" => [],
          "task" => "test_pkg/33", "index" => 1, "total" => 1, "state" => "finished", "progress" => 100, "ticker_data"=>""
        }
      end
    end
    ...
    it "should keep reference to compilation VM if create_vm fails" do
      ...
      vm = Bosh::Director::Models::Vm.filter(:agent_id => "agent-1").first
      vm.should_not be_nil
      vm.cid.should be_nil
    end
    ...
    it "should compile a package and it's dependencies" do
      ...
      check_event_log do |events|
        ...
        events[0].should == {
          "time" => anything, "stage" => "Compiling packages", "tags" => [],
          "task" => "dependency/77", "index" => 1, "total" => 2, "state" => "started", "progress" => 0, "ticker_data"=>""
        }
        events[1].should == {
          "time" => anything, "stage" => "Compiling packages", "tags" => [],
          "task" => "dependency/77", "index" => 1, "total" => 2, "state" => "finished", "progress" => 100, "ticker_data"=>""
        }
        events[2].should == {
          "time" => anything, "stage" => "Compiling packages", "tags" => [],
          "task" => "test_pkg/33", "index" => 2, "total" => 2, "state" => "started", "progress" => 0, "ticker_data"=>""
        }
        events[3].should == {
          "time" => anything, "stage" => "Compiling packages", "tags" => [],
          "task" => "test_pkg/33", "index" => 2, "total" => 2, "state" => "finished", "progress" => 100, "ticker_data"=>""
        }
      end
    end
  end
