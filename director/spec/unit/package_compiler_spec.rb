...
describe Bosh::Director::PackageCompiler do
  ...
  it "reuses compilation VMs if this option is set" do
    ...
    agents.each do |agent|
      ...
      agent.should_receive(:wait_until_ready).at_most(3).times.ordered
      agent.should_receive(:apply).with(initial_state).at_most(3).times.ordered
      agent.should_receive(:compile_package).at_most(3).times do |*args|
        name = args[2]
        dot = args[3].rindex(".")
        version, build = args[3][0..dot-1], args[3][dot+1..-1]
