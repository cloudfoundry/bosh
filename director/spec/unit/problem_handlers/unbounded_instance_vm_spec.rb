...
describe Bosh::Director::ProblemHandlers::UnboundedInstanceVm do
  before(:each) do
    ...
    @handler = Bosh::Director::ProblemHandlers::Base.create_by_type(:unbounded_instance_vm,
                                                                    @vm.id, {"job" => "test"})
  ...
  it "should raise error for resource pool vms" do
    lambda { Bosh::Director::ProblemHandlers::Base.create_by_type(:unbounded_instance_vm, @vm.id, {})}.should
    raise_error Bosh::Director::ProblemHandlers::HandlerError
  end
