...
describe Bosh::Director::Controller do
  ...
  describe "API calls" do
    ...
    describe "property management" do
      ...
      it "REST API for creating, updating, getting and deleting release properties" do
        release = Bosh::Director::Models::Release.make(:name => "appcloud")

        get "/releases/appcloud/properties/foo"
        last_response.status.should == 404

        get "/deployments/othercloud/properties/foo"
        last_response.status.should == 404

        post "/releases/appcloud/properties", {}, payload(:name => "foo", :value => "bar")
        last_response.status.should == 204
        ...
        put "/releases/appcloud/properties/foo", {}, payload(:value => "baz")
        last_response.status.should == 204

        get "/releases/appcloud/properties/foo"
        Yajl::Parser.parse(last_response.body)["value"].should == "baz"

        delete "/releases/appcloud/properties/foo"
        last_response.status.should == 204
