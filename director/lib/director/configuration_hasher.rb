...
module Bosh::Director
  class ConfigurationHasher
    ...
    def extract_prepare_templates(job_template)
      if @cached_templates.has_key?(job_template.name)
        templates = @cached_templates[job_template.name]["templates"]
        monit_template = @cached_templates[job_template.name]["monit_template"]
      else
        begin
    ...
    def hash
      ...
      @job.instances.each do |instance|
        sorted_jobs.each do |job_template|
          extract_prepare_templates(job_template)
        end
      end
      @job.instances.each do |instance|
        ...
        template_digests = {}
        sorted_jobs.each do |job_template|
          templates, monit_template = extract_prepare_templates(job_template)
