...
module Bosh::Director
  class PackageCompiler
    ...
    # done so that the "reuse_compilation_vms" option will know when to delete
    # the reused VMs.
          ...
          dependencies.each do |blocked_task|
            if blocked_task.ready_to_compile?
              @logger.info("Marking unblocked package for compilation: " +
                               "#{blocked_task.package.name}/#{stemcell_name_version(blocked_task.stemcell)}")
    ...
    def process_package(package, stemcell, job)
      ...
      if compiled_package
        ...
        @logger.info("Package \"#{package.name}\" needs to be compiled on \"#{stemcell_name_version(stemcell)}\"")
      end
