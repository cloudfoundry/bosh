module Bosh::Director::CycleHelper
  ...
  def self.check_for_cycle(vertices, options = {}, &block)
    ...
    vertices.each do |vertex|
      ...
      check_for_cycle_helper(path, vertex, connected_vertices, &block)
      result[:connected_vertices][vertex] = connected_vertices.to_a if connected_vertices
    end
    result
  end

  def self.check_for_cycle_helper(path, vertex, connected_vertices, &block)
    ...
    if edges
      edges.each do |edge|
        ...
        check_for_cycle_helper(path, edge, connected_vertices, &block)
      end
    end
    path.delete(vertex)
  end
