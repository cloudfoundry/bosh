...
module Bosh::Director
  module Jobs
    class UpdateRelease < BaseJob
        ...
        @release_manifest["packages"].each do |package_meta|
          ...
          if packages.nil? || packages.empty?
            ...
            if package
              ...
              @logger.info("Release #{@release.id} now using package #{package.inspect}")
      ...
      def copy_package_blob(blobstore_id)
        # Create a copy of the given blob
        Config.temp_path do |path|
      ...
      def create_package(package_meta)
        ...
        if package_meta["blobstore_id"]
          ...
          package.blobstore_id = copy_package_blob(package_meta["blobstore_id"])
        else
