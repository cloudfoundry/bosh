  ...
  module Jobs
    class UpdateStemcell < BaseJob
      ...
      def perform
        ...
        @event_log.track_and_log("Checking if this stemcell already exists") do
        ...
        @event_log.track_and_log("Uploading stemcell #{@stemcell_image} to the cloud") do
          stemcell.cid = @cloud.create_stemcell(@stemcell_image, @cloud_properties)
          @logger.info("Cloud created stemcell: #{stemcell.cid}")
        end

        @event_log.track_and_log("Save stemcell: #{stemcell.name}/#{stemcell.version} (#{stemcell.cid})") do
          stemcell.save
        end
