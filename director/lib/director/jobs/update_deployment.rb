  ...
  module Jobs
    class UpdateDeployment
      ...
      def prepare
        ...
        progress_and_log("Preparing", "Binding release", 1, 9)
        @deployment_plan_compiler.bind_release
        progress_and_log("Preparing", "Binding existing deployment", 2, 9)
        @deployment_plan_compiler.bind_existing_deployment
        progress_and_log("Preparing", "Binding resource pools", 3, 9)
        @deployment_plan_compiler.bind_resource_pools
        progress_and_log("Preparing", "Binding stemcells", 4, 9)
        @deployment_plan_compiler.bind_stemcells
        progress_and_log("Preparing", "Binding templates", 5, 9)
        @deployment_plan_compiler.bind_templates
        progress_and_log("Preparing", "Binding unallocated VMs", 6, 9)
        @deployment_plan_compiler.bind_unallocated_vms
        progress_and_log("Preparing", "Binding instance networks", 7, 9)
        @deployment_plan_compiler.bind_instance_networks
        progress_and_log("Preparing", "Compiling and binding packages", 8, 9)
        PackageCompiler.new(@deployment_plan).compile
        progress_and_log("Preparing", "Binding configuration", 9, 9)
      ...
      def update_resource_pools
        ...
        ThreadPool.new(:max_threads => 32).wrap do |thread_pool|
          ...
          progress_and_log("Updating Resource Pool", "Deleting Extra VMs", 1, 3)
          ...
          progress_and_log("Updating Resource Pool", "Deleting outdated VMs", 2, 3)
          resource_pool_updaters.each do |resource_pool_updater|
          ...
          progress_and_log("Updating Resource Pool", "Creating missing VMs", 3, 3)
          resource_pool_updaters.each do |resource_pool_updater|
      ...
      def update
        ...
        progress_and_log("Binding", "Binding instance VMs", 1, 1)
        @deployment_plan_compiler.bind_instance_vms

        progress_and_log("Clean-up", "Deleting no longer needed VMs", 1, 2)
        @deployment_plan_compiler.delete_unneeded_vms

        progress_and_log("Clean-up", "Deleting no longer needed instances", 2, 2)
        @deployment_plan_compiler.delete_unneeded_instances

        @logger.info("Updating jobs")
        idx = 1
        total = @deployment_plan.jobs.size

        @deployment_plan.jobs.each do |job|
          progress_and_log("Updating jobs", "Updating job: #{job.name}", idx, total)
