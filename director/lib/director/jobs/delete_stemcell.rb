    ...
    class DeleteStemcell < BaseJob
      ...
      def perform
        ...
        lock.lock do
          ...
          @event_log.begin_stage("Deleting stemcell", 1, [@name, @version])
          ...
          compiled_packages.each do |compiled_package|
            ...
            track_and_log("Deleting #{package.name}/#{package.version}") do
          ...
          @event_log.begin_stage("Deleting stemcell meta", 1, [@name, @version])
          @event_log.track("Deleting stemcell meta") do
            @stemcell.destroy
          end
        end
