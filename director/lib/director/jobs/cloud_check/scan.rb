      ...
      class Scan < BaseJob
        ...
        def scan_vm(vm)
          ...
          begin
            ...
            disk_list = agent.list_disk rescue RuntimeError
            mounted_disk_cid = disk_list.first if disk_list.kind_of?(Array)
        ...
        def add_disk_owner(disk_cid, vm_cid)
          vm_ids = @agent_disks[disk_cid]
          if vm_ids
            vm_ids << vm_cid
          else
            vm_ids = [vm_cid]
          end
          @agent_disks[disk_cid] = vm_ids
        end
