  ...
  module Jobs
    class DeleteDeployment < BaseJob
      ...
      def perform
        ...
        @event_log.begin_stage("Deleting deployment", 1, [deployment.name])
        track_and_log("Acquiring deployment lock")
        ...
        deployment_lock.lock do
          ...
          ThreadPool.new(:max_threads => 32).wrap do |pool|
            instances.each do |instance|
              pool.process do
                track_and_log("Deleting instance #{instance.job}/#{instance.index}") do
            ...
            vms.each do |vm|
              pool.process do
                track_and_log("Deleting #{vm.cid}") do
                  delete_vm(vm)
                end
              end
            end
