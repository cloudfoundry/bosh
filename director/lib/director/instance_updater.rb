module Bosh::Director
  class InstanceUpdater
    ...
    def get_new_disk
      new_disk_cid = nil
      if @instance_spec.persistent_disk_changed? && @job_spec.persistent_disk > 0
          new_disk_cid = @cloud.create_disk(@job_spec.persistent_disk, @vm.cid)
      end
      new_disk_cid
    end
    ...
    def update(options = {})
      stop
      num_retries = 0
      new_disk_cid = get_new_disk
      begin
        update_resource_pool(new_disk_cid, num_retries > 0)
        update_persistent_disk(new_disk_cid)
        ...
        @cloud.delete_disk(new_disk_cid) if !new_disk_cid.nil?
        raise
      end
    ...
    def update_resource_pool(new_disk_cid, force)
    ...
    def update_persistent_disk(disk_cid)
